...
params:
  visn_char_param: "23"
  visn_num_param: 23
title: "VISN `r params$visn_char_param` - CRH Over Time and PC Access Metrics, by Sta5a"
---
...
#-----
crh_plot_fxn_0 <- function(visn_var, care_type_var, metric){
  if (metric == "total"){
    y_axis <- paste0("Total ", care_type_var, " Encounters")
  }
  else if (metric == "crh_encounters"){
  ...
  #--
  if (metric != "crh_per_1k_total"){
  foo <- type_sta5a_df %>%
    select(fyqtr, care_type, crh_encounters, total, parent_visn) %>%
    pivot_longer(-c(fyqtr, care_type, parent_visn)) %>%
    group_by(fyqtr, care_type, parent_visn, name) %>%
    summarise(sum_val = sum(value, na.rm = T)) %>%
    filter(care_type == care_type_var & name == metric)
  }
  else {
    foo <- type_sta5a_df %>%
      select(fyqtr, care_type, crh_encounters, total, parent_visn) %>%
      group_by(fyqtr, parent_visn, care_type) %>%
      summarise(crh_encounters = sum(crh_encounters, na.rm = T),
                total = sum(total, na.rm = T)) %>%
      mutate(sum_val = crh_encounters / total * 1000) %>%
      filter(care_type == care_type_var)
  }
  #--
  ggplot(data = subset(foo, parent_visn != visn_var),
         aes(x = fyqtr, y = sum_val, group = parent_visn)) +
    geom_line(size = 1, alpha = 0.6, color = "grey") +
    geom_point(size = 1, alpha = 0.5, color = "grey") +
    geom_line(data = subset(foo, parent_visn == visn_var),
              size = 1, color = "red") +
    scale_y_continuous(limits = c(0, NA),
                       labels = scales::comma_format(accuracy = 1)) +
    theme(axis.text.x = element_text(angle = 270, vjust = 0.3),
...
### PC CRH Encounters   
```{r}
crh_plot_fxn_0(params$visn_num_param, "Primary Care", "crh_encounters")
```

#### PC CRH Encounters - I    
```{r}
crh_plot_fxn_1(visn_var = params$visn_num_param, "Primary Care", "crh_encounters")
```

#### PC CRH Encounters - II    
```{r}
crh_plot_fxn_2(visn_var = params$visn_num_param, "Primary Care", "crh_encounters")
```

### Total PC Encounters  
```{r}
crh_plot_fxn_0(params$visn_num_param, "Primary Care", "total")
```

#### Total PC Encounters - I    
```{r}
crh_plot_fxn_1(visn_var = params$visn_num_param, "Primary Care", "total")
```

#### Total PC Encounters - II    
```{r}
crh_plot_fxn_2(visn_var = params$visn_num_param, "Primary Care", "total")
```

### PC CRH Encounters per 1,000 Total PC Encounters   
```{r}
crh_plot_fxn_0(params$visn_num_param, "Primary Care", "crh_per_1k_total")
```

#### PC CRH Encounters per 1,000 Total PC Encounters - I    
```{r}
crh_plot_fxn_1(visn_var = params$visn_num_param, "Primary Care", "crh_per_1k_total")
```

#### PC CRH Encounters per 1,000 Total PC Encounters - II    
```{r}
crh_plot_fxn_2(visn_var = params$visn_num_param, "Primary Care", "crh_per_1k_total")
```

### As a Table  
```{r}
crh_table_fxn(visn_var = params$visn_num_param, "Primary Care")
```

## Primary Care Access Metrics  
### Established Patient Wait Time - I  
```{r}
access_plot_1(visn_var = params$visn_num_param, visn_var_quote = params$visn_char_param, established_pt_waitTime, "established_pt_waitTime")
```
  
### Established Patient Wait Time - II  
```{r}
access_plot_2(visn_var = params$visn_num_param, visn_var_quote = params$visn_char_param, established_pt_waitTime, "established_pt_waitTime")
```

### As a Table  
```{r}
access_table_fxn(visn_var = params$visn_num_param, "established_pt_waitTime")
```
 
### New Patient Wait Time - I  
```{r}
access_plot_1(visn_var = params$visn_num_param, visn_var_quote = params$visn_char_param, new_pt_waitTime, "new_pt_waitTime")
```
  
### New Patient Wait Time - II  
```{r}
access_plot_2(visn_var = params$visn_num_param, visn_var_quote = params$visn_char_param, new_pt_waitTime, "new_pt_waitTime")
```

### As a Table  
```{r}
access_table_fxn(visn_var = params$visn_num_param, "new_pt_waitTime")
```
 
### Observed:Expected Panel Size Ratio - I  
```{r}
access_plot_1(visn_var = params$visn_num_param, visn_var_quote = params$visn_char_param, obs_exp_panelSize_ratio, "obs_exp_panelSize_ratio")
```
  
### Observed:Expected Panel Size Ratio - II  
```{r}
access_plot_2(visn_var = params$visn_num_param, visn_var_quote = params$visn_char_param, obs_exp_panelSize_ratio, "obs_exp_panelSize_ratio")
```
 
### As a Table  
```{r}
access_table_fxn(visn_var = params$visn_num_param, "obs_exp_panelSize_ratio")
```

### Timely Care Success Rate - I  
```{r}
access_plot_1(visn_var = params$visn_num_param, visn_var_quote = params$visn_char_param, tc_pc_success_prop, "tc_pc_success_prop")
```
  
### Timely Care Success Rate - II  
```{r}
access_plot_2(visn_var = params$visn_num_param, visn_var_quote = params$visn_char_param, tc_pc_success_prop, "tc_pc_success_prop")
```

### As a Table  
```{r}
access_table_fxn(visn_var = params$visn_num_param, "tc_pc_success_prop")
``` 
...
### CPS CRH Encounters  
```{r}
crh_plot_fxn_0(params$visn_num_param, "Pharmacy", "crh_encounters")
```

#### CPS CRH Encounters - I    
```{r}
crh_plot_fxn_1(visn_var = params$visn_num_param, "Pharmacy", "crh_encounters")
```

#### CPS CRH Encounters - II    
```{r}
crh_plot_fxn_2(visn_var = params$visn_num_param, "Pharmacy", "crh_encounters")
```

### Total CPS Encounters
```{r}
crh_plot_fxn_0(params$visn_num_param, "Pharmacy", "total")
```

#### Total CPS Encounters - I    
```{r}
crh_plot_fxn_1(visn_var = params$visn_num_param, "Pharmacy", "total")
```

#### Total CPS Encounters - II    
```{r}
crh_plot_fxn_2(visn_var = params$visn_num_param, "Pharmacy", "total")
```

### CPS CRH Encounters per 1,000 Total CPS Encounters  
```{r}
crh_plot_fxn_0(params$visn_num_param, "Pharmacy", "crh_per_1k_total")
```

#### CPS CRH Encounters per 1,000 Total CPS Encounters - I    
```{r}
crh_plot_fxn_1(visn_var = params$visn_num_param, "Pharmacy", "crh_per_1k_total")
```

#### CPS CRH Encounters per 1,000 Total CPS Encounters - II    
```{r}
crh_plot_fxn_2(visn_var = params$visn_num_param, "Pharmacy", "crh_per_1k_total")
```

### As a Table  
```{r}
crh_table_fxn(visn_var = params$visn_num_param, "Pharmacy")
```