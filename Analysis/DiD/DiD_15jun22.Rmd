...
## Distribution of Established Patient Wait Times    
```{r}
p1 <- est_pt_wt %>%
...
```{r}
differences <- est_pt_wt %>%
  group_by(crh_flag, vssc_month) %>%
  summarise(est_pc_pt_wt = mean(est_pc_pt_wt, na.rm = T)) %>%
...
foo <- tibble(
  ...
  est_pc_pt_wt = c(crh_sep, (crh_sep + (noCrh_feb - noCrh_sep)))
)
#
differences %>%
  bind_rows(., foo) %>%
  mutate(est_pc_pt_wt = round(est_pc_pt_wt, 1)) %>%
  kbl(col.names = c("CRH Flag", "Month", "Average Est. Pat. WT")) %>%
...
ggplot(data = did_plot_df,
       aes(x = timepoint, y = value, group = label_name, color = label_name)) +
  geom_line(size = 1.5) +
  scale_y_continuous(limits = c(55,#plyr::round_any(min(did_plot_df$value), 5, floor), 
                                67),#plyr::round_any(max(did_plot_df$value), 5, ceiling)),
                     labels = scales::comma_format(accuracy = 1)) +
  theme(legend.position = "right",
        axis.title.x = element_blank()) +
  guides(color = guide_legend(override.aes = list(size = 3))) +
...
ggplot(data = did_plot_df,
  ...
  geom_segment(aes(x = 2.02, xend = 2.02,
               ...
               arrow = arrow(angle = 90, ends = "both", length = unit(0.125, "cm"))) +
  geom_curve(aes(x = 2.3, xend = 2.05,
                 y = (crh_sep + (noCrh_feb - noCrh_sep)) - 3.85,
                 yend = (crh_sep + (noCrh_feb - noCrh_sep)) + (crh_feb - (crh_sep + (noCrh_feb - noCrh_sep)))/2),
             color = "black",
             arrow = arrow(type = "closed", length = unit(0.25, "cm"))) +
  annotate("text",
           x = 2.125, y = (crh_sep + (noCrh_feb - noCrh_sep)) - 4,
           label = "{Difference-in-Differences}") +
  labs(y = "Established Patient Wait Time (days)")
```