---
title: "PC CRH Analysis: Draft of Step 1"
date: "`r Sys.Date()`"
...
dates <- access_metrics %>%
  filter(vssc_month <= ymd("2020-02-01")
         & vssc_month != ymd("2019-10-01")) %>%#this is where we drop October, 2019
  select(vssc_month) %>%
  distinct %>%
  arrange(vssc_month) %>%
...
# Model 1: No October, 2019  
## Established Patient WaitTime ~ Time X CRH X Period    
* This model includes `r nrow(model_1) %>% comma(accuracy = 1)` observations (i.e., rows) from `r n_distinct(model_1$sta5a)` sta5as over 13 months from January, 2019 through February, 2020 (not including October, 2019)
* This model only includes CBOCs (i.e., PC-CBOC or MS-CBOC)  
...
#
lm_1.emm <- emmeans(lm_1, ~ time*crh_flag*post_crh_implementation, at = list(time = c(1, 10, 13)))
#
...
tab_model(lm_2,
            ...
            dv.labels = "Established Patient Wait Time")
#
lm_2.emm <- emmeans(lm_2, ~ time*crh_flag*post_crh_implementation, at = list(time = c(1, 10, 13)))
#
...
tab_model(lm_3,
            ...
            dv.labels = "Established Patient Wait Time")
#
lm_3.emm <- emmeans(lm_3, ~ time*crh_flag*post_crh_implementation, at = list(time = c(1, 10, 13)))
#
...
```{r}
lm_4 <- lm(est_pc_pt_wt ~ time*crh_flag*post_crh_implementation, data = subset(model_1, scrssn_count_cat == "10,000+"))
#
lm_4$call
#
tab_model(lm_4,
            ...
            dv.labels = "Established Patient Wait Time")
#
lm_4.emm <- emmeans(lm_4, ~ time*crh_flag*post_crh_implementation, at = list(time = c(1, 10, 13)))
#
...
# Model 2: Including October, 2019  
## Established Patient WaitTime ~ Time X CRH X Period    
* This model includes `r nrow(model_2) %>% comma(accuracy = 1)` observations (i.e., rows) from `r n_distinct(model_2$sta5a)` sta5as over 14 months from January, 2019 through February, 2020   
* This model only includes CBOCs (i.e., PC-CBOC or MS-CBOC)  

### All PCMM Categories    
* There are `r model_2 %>% filter(crh_flag == "PC CRH") %>% select(sta5a) %>% n_distinct()` distinct sta5as that had at least 2 consecutive months with > 9 PC CRH encounters, and `r model_2 %>% filter(crh_flag != "PC CRH") %>% select(sta5a) %>% n_distinct()` control sta5as in this model



```{r}
lm_1 <- lm(est_pc_pt_wt ~ time*crh_flag*post_crh_implementation, data = model_2)
#
lm_1$call
#
tab_model(lm_1,
...
### 450 - 2,399 Uniques      
* There are `r model_2 %>% filter(crh_flag == "PC CRH" & scrssn_count_cat == "450 - 2,399") %>% select(sta5a) %>% n_distinct()` distinct sta5as that had at least 2 consecutive months with > 9 PC CRH encounters, and `r model_2 %>% filter(crh_flag != "PC CRH" & scrssn_count_cat == "450 - 2,399") %>% select(sta5a) %>% n_distinct()` control sta5as in this model



```{r}
lm_2 <- lm(est_pc_pt_wt ~ time*crh_flag*post_crh_implementation, data = subset(model_2, scrssn_count_cat == "450 - 2,399"))
#
lm_2$call
#
tab_model(lm_2,
...
### 2,400 - 9,999 Uniques      
* There are `r model_2 %>% filter(crh_flag == "PC CRH" & scrssn_count_cat == "2,400 - 9,999") %>% select(sta5a) %>% n_distinct()` distinct sta5as that had at least 2 consecutive months with > 9 PC CRH encounters, and `r model_2 %>% filter(crh_flag != "PC CRH" & scrssn_count_cat == "2,400 - 9,999") %>% select(sta5a) %>% n_distinct()` control sta5as in this model



```{r}
lm_3 <- lm(est_pc_pt_wt ~ time*crh_flag*post_crh_implementation, data = subset(model_2, scrssn_count_cat == "2,400 - 9,999"))
#
lm_3$call
#
tab_model(lm_3,
...
### 10,000+ Uniques      
* There are `r model_2 %>% filter(crh_flag == "PC CRH" & scrssn_count_cat == "10,000+") %>% select(sta5a) %>% n_distinct()` distinct sta5as that had at least 2 consecutive months with > 9 PC CRH encounters, and `r model_2 %>% filter(crh_flag != "PC CRH" & scrssn_count_cat == "10,000+") %>% select(sta5a) %>% n_distinct()` control sta5as in this model



```{r}
lm_4 <- lm(est_pc_pt_wt ~ time*crh_flag*post_crh_implementation, data = subset(model_2, scrssn_count_cat == "10,000+"))
#
lm_4$call
#
tab_model(lm_4,
...
# An Aside on Dropping October, 2019  
## Model 1, All PCMM Categories  
```{r}
ggplot(data = model_1,
                   aes(x = time, y = est_pc_pt_wt, group = sta5a)) +
      geom_line(alpha = 0.35) +
      geom_smooth(aes(x = time, y = est_pc_pt_wt, group = crh_flag, lty = crh_flag),
                  color = "red",
                  inherit.aes = FALSE,
                  se = TRUE,
                  method = "lm") +
...
## Model 1, 450 - 2,399 Uniques    
```{r}
ggplot(data = subset(model_1, scrssn_count_cat == "450 - 2,399"),
                   aes(x = time, y = est_pc_pt_wt, group = sta5a)) +
      geom_line(alpha = 0.35) +
      geom_smooth(aes(x = time, y = est_pc_pt_wt, group = crh_flag, lty = crh_flag),
                  color = "red",
...
## Model 2, 450 - 2,399 Uniques    
```{r}
ggplot(data = subset(model_2, scrssn_count_cat == "450 - 2,399"),
                   aes(x = time, y = est_pc_pt_wt, group = sta5a)) +
      geom_line(alpha = 0.35) +
      geom_smooth(aes(x = time, y = est_pc_pt_wt, group = crh_flag, lty = crh_flag),
                  color = "darkorchid",
...
## Model 1, 2,400 - 9,999 Uniques    
```{r}
ggplot(data = subset(model_1, scrssn_count_cat == "2,400 - 9,999"),
                   aes(x = time, y = est_pc_pt_wt, group = sta5a)) +
      geom_line(alpha = 0.35) +
      geom_smooth(aes(x = time, y = est_pc_pt_wt, group = crh_flag, lty = crh_flag),
                  color = "red",
...
## Model 2, 2,400 - 9,999 Uniques    
```{r}
ggplot(data = subset(model_2, scrssn_count_cat == "2,400 - 9,999"),
                   aes(x = time, y = est_pc_pt_wt, group = sta5a)) +
      geom_line(alpha = 0.35) +
      geom_smooth(aes(x = time, y = est_pc_pt_wt, group = crh_flag, lty = crh_flag),
                  color = "darkorchid",
...
## Model 1, 10,000+ Uniques    
```{r}
ggplot(data = subset(model_1, scrssn_count_cat == "10,000+"),
                   aes(x = time, y = est_pc_pt_wt, group = sta5a)) +
      geom_line(alpha = 0.35) +
      geom_smooth(aes(x = time, y = est_pc_pt_wt, group = crh_flag, lty = crh_flag),
                  color = "red",
...
## Model 2, 10,000+ Uniques    
```{r}
ggplot(data = subset(model_2, scrssn_count_cat == "10,000+"),
                   aes(x = time, y = est_pc_pt_wt, group = sta5a)) +
      geom_line(alpha = 0.35) +
      geom_smooth(aes(x = time, y = est_pc_pt_wt, group = crh_flag, lty = crh_flag),
                  color = "darkorchid",
