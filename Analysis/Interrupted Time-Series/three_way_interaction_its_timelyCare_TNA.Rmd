...
## Timely Care Success ~ Time X CRH X Period    
...
ggplot(data = model_1a,
  ...
  geom_smooth(data = subset(model_1a, vssc_month > ymd("2019-09-30")),
              aes(x = vssc_month, y = tc_success_prop, group = crh_flag, lty = crh_flag),
              color = "darkorchid3",
              inherit.aes = FALSE,
              se = FALSE,
              method = "lm",
...
```{r}
options(width = 400)
lm_1 <- lm(tc_success_prop ~ time*crh_flag*post_crh_implementation, data = model_1)
#
lm_1$call
#
tab_model(lm_1,
            show.se = TRUE,
            dv.labels = "Timely Care Success Rate",
...
```{r}
options(width = 400)
lm_2 <- lm(tc_success_prop ~ time*crh_flag*post_crh_implementation, data = subset(model_1, scrssn_count_cat == "450 - 2,399"))
#
lm_2$call
#
tab_model(lm_2,
...
```{r}
options(width = 400)
lm_3 <- lm(tc_success_prop ~ time*crh_flag*post_crh_implementation, data = subset(model_1, scrssn_count_cat == "2,400 - 9,999"))
#
lm_3$call
#
tab_model(lm_3,
...
```{r}
options(width = 400)
lm_4 <- lm(tc_success_prop ~ time*crh_flag*post_crh_implementation, 
...
## Third Next Available Appointment ~ Time X CRH X Period    
* This model includes `r nrow(model_1) %>% comma(accuracy = 1)` observations (i.e., rows) from `r n_distinct(model_1$sta5a)` sta5as over 17 months from October, 2018 through February, 2020   
...
ggplot(data = model_1a,
  ...
  geom_line(alpha = 0.35) +
  geom_smooth(data = subset(model_1a, vssc_month > ymd("2019-09-30")),
              aes(x = vssc_month, y = third_next_avail, group = crh_flag, lty = crh_flag),
              color = "darkorchid3",
              inherit.aes = FALSE,
              se = FALSE,
              method = "lm",
...
```{r}
options(width = 400)
lm_1 <- lm(third_next_avail ~ time*crh_flag*post_crh_implementation, data = model_1)
#
lm_1$call
#
tab_model(lm_1,
            show.se = TRUE,
            dv.labels = "Third Next Available Appointment",
          pred.labels = c("Intercept","Time", "PC CRH (vs. No PC CRH)",
                          "Post Implementation (vs. Pre)",
                          "Time X PC CRH (vs. No PC CRH)",
                          "Time X Post Implementation (vs. Pre)",
...
```{r}
options(width = 400)
lm_2 <- lm(third_next_avail ~ time*crh_flag*post_crh_implementation, data = subset(model_1, scrssn_count_cat == "450 - 2,399"))
#
lm_2$call
#
tab_model(lm_2,
...
```{r}
options(width = 400)
lm_3 <- lm(third_next_avail ~ time*crh_flag*post_crh_implementation, data = subset(model_1, scrssn_count_cat == "2,400 - 9,999"))
#
lm_3$call
#
tab_model(lm_3,
...
```{r}
options(width = 400)
lm_4 <- lm(third_next_avail ~ time*crh_flag*post_crh_implementation, 
           data = subset(model_1, scrssn_count_cat == "10,000+"))
#
lm_4$call
#
