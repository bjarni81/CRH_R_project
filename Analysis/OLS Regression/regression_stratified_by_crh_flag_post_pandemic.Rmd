---
title: "PC CRH Analysis: Comparing Regression Slopes"
date: "`r Sys.Date()`"
...
table_fxn <- function(access_metric, scrssn_cat){
  ...
  if(missing(scrssn_cat)){
    ...
    if(str_detect(access_metric, "tc_success") != T){
      ...
      #--
      value_t1 <- tibble(
        table_labs = paste0("Predicted ", y_axis_lab, " in March, 2021"),
      ...
      #--
      value_t17 <- tibble(
        table_labs = paste0("Predicted ", y_axis_lab, " in July, 2022"),
...
regression_fxn <- function(access_metric, pcmm_cat){
  ...
  #==
  reg_df_crh <- if(missing(pcmm_cat)){model_1a}
  else {model_1a %>%
    filter(scrssn_count_cat == !!pcmm_cat)}
  #==
...
regression_table_fxn <- function(access_metric, pcmm_cat){
  ...
  #--
  reg_df_noCrh <- if(missing(pcmm_cat)){model_1a %>%
    filter(crh_flag == "No PC CRH")}
  else {model_1a %>%
    filter(scrssn_count_cat == !!pcmm_cat 
           & crh_flag == "No PC CRH")}
...
regression_table_fxn("est_pc_pt_wt")
```
...
```{r}
regression_table_fxn("est_pc_pt_wt", "450 - 2,399")
```
...
ggplot(data = model_1a,
    ...
    scale_x_date() +
    geom_line(alpha = 0.35) +
    geom_smooth(aes(x = vssc_month, y = new_pc_pt_wt, group = crh_flag, lty = crh_flag),
                color = "red",
                inherit.aes = FALSE,
                se = TRUE,
                method = "lm") +
...
regression_table_fxn("new_pc_pt_wt")
```
...
ggplot(data = model_1a,
    ...
    scale_x_date() +
    geom_line(alpha = 0.35) +
    geom_smooth(aes(x = vssc_month, y = third_next_avail, group = crh_flag, lty = crh_flag),
                color = "red",
                inherit.aes = FALSE,
                se = TRUE,
                method = "lm") +
...
```{r}
regression_fxn("third_next_avail")
regression_table_fxn("third_next_avail")
```
...
```{r}
ggplot(data = model_1a,
               aes(x = vssc_month, y = tc_success_prop, group = sta5a)) +
    geom_vline(xintercept = vline_xint,
               lty = 3) +
    scale_x_date() +
    geom_line(alpha = 0.35) +
    geom_smooth(aes(x = vssc_month, y = tc_success_prop, group = crh_flag, lty = crh_flag),
                color = "red",
                inherit.aes = FALSE,
                se = TRUE,
                method = "lm") +
...
```{r}
regression_fxn("tc_success_prop")
regression_table_fxn("tc_success_prop")
```
