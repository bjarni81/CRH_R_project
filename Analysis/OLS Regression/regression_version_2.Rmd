---
title: "PC CRH Analysis: After July 19th CRH Meeting"
date: "`r Sys.Date()`"
...
#=================
model_2a <- access_metrics %>%
      inner_join(., sta5as_to_include) %>%
      filter(vssc_month <= ymd("2020-02-01")) %>%
      left_join(., dates) %>%#this is the difference between 1a and 2a
      left_join(., crh_flag %>% select(sta5a, first_6_mos_w_10_flag)) %>%
      mutate(all_first_6_months = if_else(sta5a %in% ps_groups_w_gt9, 
                                          "All First 6 Months", "Not All First 6 Months"),
             tna_drop = if_else(third_next_avail > 100, TRUE, FALSE)) %>%
...
```{r}
table_fxn <- function(access_metric, scrssn_cat, which_model){
  fwhich_model <- get(paste0("model_", which_model))
    ...
    if(missing(scrssn_cat)){
      ttest_uniques <- t.test(scrssn_count_avg_fy19 ~ crh_flag, data = fwhich_model)
      ...
      ttest_value <- if (str_detect(access_metric, "est") == T){
        t.test(est_pc_pt_wt ~ crh_flag, data = fwhich_model)
      }
      else if (str_detect(access_metric, "new") == T){
        t.test(new_pc_pt_wt ~ crh_flag, data = fwhich_model)
      }
      else if (str_detect(access_metric, "third") == T){
        t.test(third_next_avail ~ crh_flag, data = fwhich_model)
      }
      else if (str_detect(access_metric, "tc_success_prop") == T){
        t.test(tc_success_prop ~ crh_flag, data = fwhich_model)
      }
      ...
      if(str_detect(access_metric, "tc_success") != T){
        ...
        #--
        value_t1 <- tibble(
          table_labs = paste0("Predicted ", y_axis_lab, " in May, 2019"),
...
#===============================
plot_fxn <- function(access_metric, pcmm_cat, which_model){
  access_metric_var <- ensym(access_metric)
  #
  y_axis_lab <- if (str_detect(access_metric, "est") == T){
    "Established Patient Wait Time"
  ...
  model_1a <- if (str_detect(access_metric, "third") != T){access_metrics %>%
                    ...
                    filter(scrssn_count_cat == !!pcmm_cat)}
  #-----
  model_2a <- if (str_detect(access_metric, "third") != T){access_metrics %>%
                    select(sta5a, vssc_month, access_metric_var) %>%
                    inner_join(., sta5as_to_include) %>%
                    filter(vssc_month <= ymd("2020-02-01")) %>%
                    left_join(., dates) %>%
  ...
  if (str_detect(access_metric, "tc_") == T){
    print(p + scale_y_continuous(labels = percent_format(accuracy = 1)))
  }
  else {
    print(p)
  }
...
#------------------------
regression_fxn <- function(access_metric, pcmm_cat, which_model){
  fwhich_model <- get(paste0("model_", which_model))
...
#=============
regression_table_fxn <- function(access_metric, pcmm_cat, which_model){
  fwhich_model <- get(paste0("model_", which_model))
  reg_label <- if (str_detect(access_metric, "est") == T){
    "Established Patient Wait Time"
  }
...
## Model Without October, 2019  
```{r est_table1, results = 'asis'}
table_fxn("est_pc_pt_wt", "450 - 2,399", which_model = "1a")
```

```{r est_plot1}
plot_fxn("est_pc_pt_wt", "450 - 2,399", which_model = "1a")
```

```{r est_reg1}
regression_fxn("est_pc_pt_wt", "450 - 2,399", which_model = "1a")
```

```{r}
regression_table_fxn("est_pc_pt_wt", "450 - 2,399", which_model = "1a")
```
...
```{r}
regression_table_fxn("est_pc_pt_wt", "2,400 - 9,999")
```
...
```{r}
regression_table_fxn("est_pc_pt_wt", "10,000+")
```

# New Patient Wait Time  
## 450 - 2,399 Uniques in PCMM  
```{r new_table1, results = 'asis'}
table_fxn("new_pc_pt_wt", "450 - 2,399")
```

```{r new_plot1}
plot_fxn("new_pc_pt_wt", "450 - 2,399")
```

```{r new_reg1}
regression_fxn("new_pc_pt_wt", "450 - 2,399")
```

```{r}
regression_table_fxn("new_pc_pt_wt", "450 - 2,399")
```

## 2,400 - 9,999 Uniques in PCMM  
```{r new_table2, results = 'asis'}
table_fxn("new_pc_pt_wt", "2,400 - 9,999")
```

```{r new_plot2}
plot_fxn("new_pc_pt_wt", "2,400 - 9,999")
```

```{r new_reg2}
regression_fxn("new_pc_pt_wt", "2,400 - 9,999")
```

```{r}
regression_table_fxn("new_pc_pt_wt", "2,400 - 9,999")
```

## 10,000+ Uniques in PCMM  
```{r new_table3, results = 'asis'}
table_fxn("new_pc_pt_wt", "10,000+")
```

```{r new_plot3}
plot_fxn("new_pc_pt_wt", "10,000+")
```

```{r new_reg3}
regression_fxn("new_pc_pt_wt", "10,000+")
```

```{r}
regression_table_fxn("new_pc_pt_wt", "10,000+")
```

# Third Next Available    
## 450 - 2,399 Uniques in PCMM  
```{r tna_table1, results = 'asis'}
table_fxn("third_next_avail", "450 - 2,399")
```

```{r tna_plot1}
plot_fxn("third_next_avail", "450 - 2,399")
```

```{r tna_reg1}
regression_fxn("third_next_avail", "450 - 2,399")
```

```{r}
regression_table_fxn("third_next_avail", "450 - 2,399")
```

## 2,400 - 9,999 Uniques in PCMM  
```{r tna_table2, results = 'asis'}
table_fxn("third_next_avail", "2,400 - 9,999")
```

```{r tna_plot2}
plot_fxn("third_next_avail", "2,400 - 9,999")
```

```{r tna_reg2}
regression_fxn("third_next_avail", "2,400 - 9,999")
```

```{r}
regression_table_fxn("third_next_avail", "2,400 - 9,999")
```

## 10,000+ Uniques in PCMM  
```{r tna_table3, results = 'asis'}
table_fxn("third_next_avail", "10,000+")
```

```{r tna_plot3}
plot_fxn("third_next_avail", "10,000+")
```

```{r tna_reg3}
regression_fxn("third_next_avail", "10,000+")
```

```{r}
regression_table_fxn("third_next_avail", "10,000+")
```


# Timely Care Success Rate      
## 450 - 2,399 Uniques in PCMM  
```{r timelyCare_table1, results = 'asis'}
table_fxn("tc_success_prop", "450 - 2,399")
```

```{r timelyCare_plot1}
plot_fxn("tc_success_prop", "450 - 2,399")
```

```{r timelyCare_reg1}
regression_fxn("tc_success_prop", "450 - 2,399")
```

```{r}
regression_table_fxn("tc_success_prop", "450 - 2,399")
```

## 2,400 - 9,999 Uniques in PCMM  
```{r timelyCare_table2, results = 'asis'}
table_fxn("tc_success_prop", "2,400 - 9,999")
```

```{r timelyCare_plot2}
plot_fxn("tc_success_prop", "2,400 - 9,999")
```

```{r timelyCare_reg2}
regression_fxn("tc_success_prop", "2,400 - 9,999")
```

```{r}
regression_table_fxn("tc_success_prop", "2,400 - 9,999")
```

## 10,000+ Uniques in PCMM  
```{r timelyCare_table3, results = 'asis'}
table_fxn("tc_success_prop", "10,000+")
```

```{r timelyCare_plot3}
plot_fxn("tc_success_prop", "10,000+")
```

```{r timelyCare_reg3}
regression_fxn("tc_success_prop", "10,000+")
```

```{r}
regression_table_fxn("tc_success_prop", "10,000+")
```
