---
title: "Post-COVID PC CRH Analysis: Meaningful Use vs. Penetration Rate"
date: "`r Sys.Date()`"
...
analytic_df2 <- vast %>%
  ...
  left_join(., pen_rate %>%
              mutate(pen_rate_per_100 = pc_crh_per_1k_total_pc / 10,
                     pen_rate_per_10 = pc_crh_per_1k_total_pc / 100)) %>%
...
* This is a sensitivity analysis contrasting the PC CRH penetration rate vs. a monthly meaningful use indicator 
* Because as of the time of running this report we haven't made a decision about panel fullness vs. gap metric I am leaving both of those out of the models below
...
wbm_fxn <- function(outcome){
  ...
  wb_fmla_trtd <- as.formula(paste0(outcome, " ~ treated_month + time_t ", 
                               ...
                               "+ team_pcp_ap_fte_total + pct_male + pct_white + pct_rural + s_abbr2 + scrssn_count_cat + parent_visn", 
                               " | (1|sta5a)"))
  wb_fmla_pen <- as.formula(paste0(outcome, " ~ pen_rate_per_10 + time_t ", 
                               "+ adi_natRnk_avg + avg_age_oct1_2022 + avg_driveTime + nosos_risk_score",
                               "+ team_pcp_ap_fte_total + pct_male + pct_white + pct_rural + scrssn_count_cat", 
                               "| treated_ever", 
                               "+ adi_natRnk_avg + avg_age_oct1_2022 + avg_driveTime + nosos_risk_score",
  ...
  #=====
  within_trtd_sum <- if({{outcome}} == "new_0_20_binary_success"){mod_trtd$within_table %>%
    rownames_to_column() %>%
      mutate(`Est.` = exp(`Est.`))}
  else {mod_trtd$within_table %>%
    rownames_to_column()}
  #
  between_trtd_sum <- if({{outcome}} == "new_0_20_binary_success"){mod_trtd$between_table %>%
    rownames_to_column() %>%
      mutate(`Est.` = exp(`Est.`))}
  else {mod_trtd$between_table %>%
    rownames_to_column()}
  #
  within_pen_sum <- if({{outcome}} == "new_0_20_binary_success"){mod_pen$within_table %>%
    rownames_to_column() %>%
      mutate(`Est.` = exp(`Est.`))}
  else {mod_pen$within_table %>%
    rownames_to_column()}
  #
  between_pen_sum <- if({{outcome}} == "new_0_20_binary_success"){mod_pen$between_table %>%
    rownames_to_column() %>%
      mutate(`Est.` = exp(`Est.`))}
  else {mod_pen$between_table %>%
    rownames_to_column()}
  #
  ...
  within_table <- within_og_sum %>%
    full_join(., within_trtd_sum,
              by = "rowname") %>%
    full_join(., within_pen_sum,
              by = "rowname") %>%
    select(rowname, 
    ...
    add_header_above(c(" ", "Original Analysis" = 2, "Meaningful Use" = 2, "Penetration Rate" = 2),
                     bold = T,
                     font_size = 18)
  #=======
  between_table <- between_og_sum %>%
    full_join(., between_trtd_sum,
              by = "rowname") %>%
    full_join(., between_pen_sum,
              by = "rowname") %>%
    select(rowname, 
           Est..x, p.x, Est..y, p.y, Est., p) %>%
    mutate(across(contains("Est"), ~comma(.x, accuracy = 0.001)),
