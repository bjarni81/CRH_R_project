---
title: "Post-COVID PC CRH Analysis: Categorical Gap vs. Panel Fullness"
date: "`r Sys.Date()`"
...
* This is a sensitivity analysis contrasting __categorical__ gap metric vs panel fullness 
...
wbm_fxn <- function(outcome){
  ...
  wb_fmla_neither <- as.formula(paste0(outcome, " ~ treated_month + time_t ", 
  ...
  between_pf_sum <- if({{outcome}} == "new_0_20_binary_success"){mod_pf$between_table %>%
      ...
      rownames_to_column()}
  #
  within_neither_sum <- if({{outcome}} == "new_0_20_binary_success"){mod_neither$within_table %>%
      rownames_to_column() %>%
      mutate(`Est.` = exp(`Est.`))}
  else {mod_neither$within_table %>%
      rownames_to_column()}
  #
  between_neither_sum <- if({{outcome}} == "new_0_20_binary_success"){mod_neither$between_table %>%
      rownames_to_column() %>%
      mutate(`Est.` = exp(`Est.`))}
  else {mod_neither$between_table %>%
      rownames_to_column()}
  #----
  within_table <- within_neither_sum %>%
    ...
    add_header_above(c(" ", "Neither" = 2, "Gap Metric" = 2, "Panel Fullness" = 2),
                     bold = T,
                     font_size = 18)
  #=======
  between_table <- between_neither_sum %>%
