---
title: "Post-COVID PC CRH Analysis: Effect of Removing Drive Time"
date: "`r Sys.Date()`"
...
analytic_df2 <- vast %>%
  ...
  mutate(treated_ever = relevel(as.factor(treated_ever), ref = "0"),
         ...
         crh_corr_fte = team_pcp_ap_fte_total - crh_pcp_ap_fte) %>%
  fastDummies::dummy_cols(., select_columns = c("parent_visn", "s_abbr")) %>%
  arrange(sta5a, vssc_month) %>%
  group_by(sta5a) %>%
  fill(c(avg_driveTime, avg_age_oct1_2022),
       .direction = "downup") %>% ungroup
#
sta5as_to_include <- analytic_df2 %>% group_by(sta5a) %>%
  summarise(missing_count = sum(is.na(avg_driveTime), na.rm = T)) %>%
  filter(missing_count == 36) %>%
  select(sta5a)
#--
analytic_panel_noMatch <- analytic_df2 %>%
  filter(sta5a %ni% sta5as_to_include$sta5a) %>%
  panelr::panel_data(.,
                     id = sta5a,
                     wave = time)
```
...
* This is the "final" time series model
* We are moving forward with a __un-matched__ __Within-Between__ model
* We are using last observation carried forward for __age__ and __drive time__
    + This analysis removes __average drive time__ to reclaim more sta5as that get dropped because of missingness
...
* Description of the unmatched cohort for the W-B model 
    + The first observation in the data occurs on `r min(analytic_panel_noMatch$vssc_month)`
    + The last observation in the data occurs on `r max(analytic_panel_noMatch$vssc_month)`
    + There are `r nrow(analytic_panel_noMatch) %>% comma()` rows in the data
    + These rows come from `r analytic_panel_noMatch %>% ungroup %>% select(sta5a) %>% n_distinct()` sta5as
      + `r analytic_panel_noMatch %>% ungroup %>% select(sta5a, treated_ever) %>% distinct() %>% ungroup %>% summarise(tot = sum(if_else(treated_ever == "1", 1, 0)))` of these met our criteria for PC CRH 
      + `r analytic_panel_noMatch %>% ungroup %>% select(sta5a, s_abbr) %>% distinct() %>% ungroup %>% summarise(tot = sum(str_detect(s_abbr, "CBOC") == T))` of these are CBOCs
      + `r analytic_panel_noMatch %>% ungroup %>% select(sta5a, s_abbr) %>% distinct() %>% ungroup %>% summarise(tot = sum(str_detect(s_abbr, "HCC|VAMC") == T))` of these are HCCs or VAMCs
      + `r analytic_panel_noMatch %>% ungroup %>% select(sta5a, s_abbr) %>% distinct() %>% summarise(tot = sum(str_detect(s_abbr, "OOS") == T))` of these are OOS
...
wbm_fxn <- function(outcome){
  fmla_og <- as.formula(paste0(outcome, " ~ treated_month + time_t ", 
                               ...
                               " | (1|sta5a)"))
  fmla_no_driveTime <- as.formula(paste0(outcome, " ~ treated_month + time_t ", 
                               "+ gap_metric_cat + adi_natRnk_avg + avg_age_oct1_2022 + nosos_risk_score",
                               "+ team_pcp_ap_fte_total + pct_male + pct_white + pct_rural + scrssn_count_cat", 
                               "| treated_ever", 
                               "+ gap_metric_cat + adi_natRnk_avg + avg_age_oct1_2022 + nosos_risk_score",
                               "+ team_pcp_ap_fte_total + pct_male + pct_white + pct_rural + s_abbr2 + scrssn_count_cat + parent_visn", 
  ...
  #=====
  within_og_sum <- if({{outcome}} == "new_0_20_binary_success"){mod_og %>% summary %$% within_table %>%
      rownames_to_column() %>%
      mutate(`Est.` = exp(`Est.`))}
  else {mod_og %>% summary %$% within_table %>%
      rownames_to_column()}
  #
  between_og_sum <- if({{outcome}} == "new_0_20_binary_success"){mod_og %>% summary %$% between_table %>%
      rownames_to_column() %>%
      mutate(`Est.` = exp(`Est.`))}
  else {mod_og %>% summary %$% between_table %>%
      rownames_to_column()}
  #
  within_no_driveTime_sum <- if({{outcome}} == "new_0_20_binary_success"){mod_no_driveTime %>% summary %$% within_table %>%
      rownames_to_column() %>%
      mutate(`Est.` = exp(`Est.`))}
  else {mod_no_driveTime %>% summary %$% within_table %>%
      rownames_to_column()}
  #
  between_no_driveTime_sum <- if({{outcome}} == "new_0_20_binary_success"){mod_no_driveTime %>% summary %$% between_table %>%
      rownames_to_column() %>%
      mutate(`Est.` = exp(`Est.`))}
  else {mod_no_driveTime %>% summary %$% between_table %>%
      rownames_to_column()}
  #----
  within_table <- within_og_sum %>%
    full_join(., within_no_driveTime_sum,
              by = "rowname") %>%
    select(rowname, 
           Est..x, p.x, Est..y, p.y) %>%
    mutate(across(contains("Est"), ~comma(.x, accuracy = 0.001)),
           across(contains("p"), ~if_else(.x < 0.001, "< 0.001", comma(.x, accuracy = 0.001)))) %>%
    kbl(col.names = c("Covariate", col_name, "p-value", col_name, "p-value"),
        align = c("l", rep("r", 4)),
    ...
    add_header_above(c(" ", "Original Analysis" = 2, "CRH-Corrected FTE" = 2),
                     bold = T,
                     font_size = 18)
  #=======
  between_table <-  between_og_sum %>%
    full_join(., between_no_driveTime_sum,
              by = "rowname") %>%
    select(rowname, 
           Est..x, p.x, Est..y, p.y) %>%
    mutate(across(contains("Est"), ~comma(.x, accuracy = 0.001)),
  ...
  #
  aic_tab <- tibble(col_1 = c(" ", "Original", "No Drive Time"),
         col_2 = c("AIC", comma(AIC(mod_og), accuracy = 0.1), comma(AIC(mod_no_driveTime), accuracy = 0.1)),
         col_3 = c("BIC",comma(BIC(mod_og), accuracy = 0.1), comma(BIC(mod_no_driveTime), accuracy = 0.1))) %>%
    kbl(col.names = c("", "Original", "No Drive Time"),
        align = c("l", "r", "r")) %>%
    kable_classic("striped",
                  full_width = F) %>%
    row_spec(0, bold = TRUE)
  #
  return(list(slot_1 = within_table, slot_2 = between_table, slot_3 = aic_tab))
}
```
