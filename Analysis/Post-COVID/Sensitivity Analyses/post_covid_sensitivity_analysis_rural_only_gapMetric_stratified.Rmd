---
title: "Post-COVID PC CRH Analysis: Rural Clinics, Stratified by Gap Metric"
date: "`r Sys.Date()`"
...
analytic_df2 <- vast %>%
  ...
  left_join(., vast %>%
              select(sta5a, s_abbr, s_abbr2, parent_visn, urh_vast) %>%
...
analytic_panel_noMatch <- analytic_df2 %>%
  ...
  filter(urh2 == "Rural")
#
analytic_panel_noMatch_all <- analytic_df2 %>%
  panelr::panel_data(.,
                     id = sta5a,
                     wave = time)
```
...
* This is a _post-COVID_ sub-analysis

## Decision Points  

* This is the post-COVID analysis among __rural clinics,__ stratified by the sta5a-specific average gap metric over the 36 months of our study
* In the tables below the "Original Analysis" column includes all sta5as, not only rural
...
mean_gap %>%
  left_join(., vast %>%
              mutate(urh2 = if_else(urh_vast == "R", "Rural", "Urban")) %>%
              select(sta5a, urh2)) %>%
  tabyl(gap_cat, urh2) %>%
  adorn_percentages("col") %>%
  adorn_pct_formatting() %>%
  adorn_ns("front") %>%
  kbl(col.names = c("", "Rural", "Urban"),
      align = c("c", "r", "r")) %>%
  kable_classic("striped",
                full_width = FALSE) %>%
  row_spec(0, bold = T) 
...
high_sta5a <- mean_gap %>%
  ...
  pull(sta5a)
#
low_panel <- analytic_panel_noMatch %>%
  filter(sta5a %in% low_sta5a)
#
med_panel <- analytic_panel_noMatch %>%
  filter(sta5a %in% med_sta5a)
#
high_panel <- analytic_panel_noMatch %>%
  filter(sta5a %in% high_sta5a)
#
wbm_fxn <- function(outcome){
  fmla_og <- as.formula(paste0(outcome, " ~ treated_month + time_t ", 
                               ...
                               " | (1|sta5a)"))
  fmla <- as.formula(paste0(outcome, " ~ treated_month + time_t ", 
                               " + adi_natRnk_avg + avg_age_oct1_2022 + avg_driveTime + nosos_risk_score",
                               "+ team_pcp_ap_fte_total + pct_male + pct_white + pct_rural + scrssn_count_cat", 
                               "| treated_ever", 
                               " + adi_natRnk_avg + avg_age_oct1_2022 + avg_driveTime + nosos_risk_score",
                               "+ team_pcp_ap_fte_total + pct_male + pct_white + pct_rural + s_abbr2 + scrssn_count_cat + parent_visn", 
  ...
  mod_og <- 
    ...
    {panelr::wbm(fmla_og,
                 data = analytic_panel_noMatch_all,
                 family = "binomial",
                 pvals = T,
                 pR2 = F,
  ...
  between_og_sum <- if({{outcome}} == "new_0_20_binary_success"){mod_og$between_table %>%
    ...
    rownames_to_column()}
  #
  within_low_sum <- if({{outcome}} == "new_0_20_binary_success"){mod_low$within_table %>%
    rownames_to_column() %>%
      mutate(`Est.` = exp(`Est.`))}
  else {mod_low$within_table %>%
    rownames_to_column()}
  #
  between_low_sum <- if({{outcome}} == "new_0_20_binary_success"){mod_low$between_table %>%
    rownames_to_column() %>%
      mutate(`Est.` = exp(`Est.`))}
  else {mod_low$between_table %>%
    rownames_to_column()}
  #
  within_med_sum <- if({{outcome}} == "new_0_20_binary_success"){mod_med$within_table %>%
    rownames_to_column() %>%
      mutate(`Est.` = exp(`Est.`))}
  else {mod_med$within_table %>%
    rownames_to_column()}
  #
  between_med_sum <- if({{outcome}} == "new_0_20_binary_success"){mod_med$between_table %>%
    rownames_to_column() %>%
      mutate(`Est.` = exp(`Est.`))}
  else {mod_med$between_table %>%
    rownames_to_column()}
  #
  within_high_sum <- if({{outcome}} == "new_0_20_binary_success"){mod_high$within_table %>%
      rownames_to_column() %>%
      mutate(`Est.` = exp(`Est.`))}
  else {mod_high$within_table %>%
      rownames_to_column()}
  #
  between_high_sum <- if({{outcome}} == "new_0_20_binary_success"){mod_high$between_table %>%
      rownames_to_column() %>%
      mutate(`Est.` = exp(`Est.`))}
  else {mod_high$between_table %>%
      rownames_to_column()}
  #----
  within_table <- within_og_sum %>%
    full_join(., within_low_sum,
              by = "rowname") %>%
    full_join(., within_med_sum,
              by = "rowname") %>%
    full_join(., within_high_sum,
              by = "rowname") %>%
    select(rowname, 
           Est..x, p.x, Est..y, p.y, Est..x.x, p.x.x, Est..y.y, p.y.y) %>%
    mutate(across(contains("Est"), ~comma(.x, accuracy = 0.001)),
           across(contains("p"), ~if_else(.x < 0.001, "< 0.001", comma(.x, accuracy = 0.001)))) %>%
    kbl(col.names = c("Covariate", rep(c(col_name, "p-value"), 4)),
        align = c("l", rep("r", 8)),
    ...
    add_header_above(c(" ", "Original Analysis" = 2, "Gap Metric < 1.0" = 2, "Gap Metric 1.0-1.2" = 2, "Gap Metric > 1.2" = 2),
                     bold = T,
                     font_size = 18)
  #=======
  between_table <-  between_og_sum %>%
    full_join(., between_low_sum,
              by = "rowname") %>%
    full_join(., between_med_sum,
              by = "rowname") %>%
    full_join(., between_high_sum,
              by = "rowname") %>%
    select(rowname, 
           Est..x, p.x, Est..y, p.y, Est..x.x, p.x.x, Est..y.y, p.y.y) %>%
    mutate(across(contains("Est"), ~comma(.x, accuracy = 0.001)),
    ...
    kbl(col.names = c("Covariate", rep(c(col_name, "p-value"), 4)),
        align = c("l", rep("r", 8)),
        caption = "<span style='font-size:22px'><b>Between</b></span>") %>%
    kable_classic("striped",
                  full_width = F) %>%
    row_spec(0, bold = TRUE) %>%
    column_spec(c(3, 5, 7), border_right = TRUE) %>%
