---
title: "Post-COVID PC CRH Analysis: Stratified by Panel Fullness"
date: "`r Sys.Date()`"
...
* This is the post-COVID analysis stratified by the sta5a-specific average panel fullness over the 36 months of our study
...
mean_pf <- analytic_df2 %>%
  group_by(sta5a) %>%
  summarise(mean_pf = mean(panel_fullness_pct, na.rm = T))
low_sta5a <- mean_pf %>%
  filter(mean_pf < 80) %>%
  pull(sta5a)
med_sta5a <- mean_pf %>%
...
wbm_fxn <- function(outcome){
  fmla_og <- as.formula(paste0(outcome, " ~ treated_month + time_t ", 
                               "+ panel_fullness_cat + adi_natRnk_avg + avg_age_oct1_2022 + avg_driveTime + nosos_risk_score",
  ...
  within_table <- within_og_sum %>%
    ...
    add_header_above(c(" ", "Original Analysis" = 2, "Panel Fullness > 105%" = 2, "Panel Fullness 80-105%" = 2, 
                       "Panel Fullness < 80%" = 2),
                     bold = T,
                     font_size = 18)
  #=======
