---
title: "Post-COVID PC CRH Analysis: Stratified by Rurality"
date: "`r Sys.Date()`"
...
* This is the post-COVID analysis stratified by the rurality of each sta5a (as determined by VAST)
* `r analytic_df2 %>% select(sta5a, urh2) %>% distinct() %>% summarise(count = sum(if_else(urh2 == "Rural", 1, 0)))` out of 980 (`r (374/980) %>% percent(., accuracy = 0.1)`) sta5as are designated "Rural"
* The remaining `r analytic_df2 %>% select(sta5a, urh2) %>% distinct() %>% summarise(count = sum(if_else(urh2 != "Rural", 1, 0)))` (`r (606/980) %>% percent(., accuracy = 0.1)`) sta5as are designated "Urban"
...
```{r fxns}
rural_panel <- analytic_panel_noMatch %>%
  filter(urh2 == "Rural")
#
urban_panel <- analytic_panel_noMatch %>%
  filter(urh2 == "Urban")
#
wbm_fxn <- function(outcome){
  ...
  between_og_sum <- if({{outcome}} == "new_0_20_binary_success"){mod_og$between_table %>%
    ...
    rownames_to_column()}
  #
  within_urban_sum <- if({{outcome}} == "new_0_20_binary_success"){mod_urban$within_table %>%
    rownames_to_column() %>%
      mutate(`Est.` = exp(`Est.`))}
  else {mod_urban$within_table %>%
    rownames_to_column()}
  #
  between_urban_sum <- if({{outcome}} == "new_0_20_binary_success"){mod_urban$between_table %>%
    rownames_to_column() %>%
      mutate(`Est.` = exp(`Est.`))}
  else {mod_urban$between_table %>%
    rownames_to_column()}
  #
  within_rural_sum <- if({{outcome}} == "new_0_20_binary_success"){mod_rural$within_table %>%
    rownames_to_column() %>%
      mutate(`Est.` = exp(`Est.`))}
  else {mod_rural$within_table %>%
    rownames_to_column()}
  #
  between_rural_sum <- if({{outcome}} == "new_0_20_binary_success"){mod_rural$between_table %>%
    rownames_to_column() %>%
      mutate(`Est.` = exp(`Est.`))}
  else {mod_rural$between_table %>%
    rownames_to_column()}
  #----
  within_table <- within_og_sum %>%
    full_join(., within_urban_sum,
              by = "rowname") %>%
    full_join(., within_rural_sum,
              by = "rowname") %>%
    select(rowname, 
    ...
    add_header_above(c(" ", "Original Analysis" = 2, "Urban" = 2, "Rural" = 2),
                     bold = T,
                     font_size = 18)
  #=======
  between_table <-  between_og_sum %>%
    full_join(., between_urban_sum,
              by = "rowname") %>%
    full_join(., between_rural_sum,
              by = "rowname") %>%
    select(rowname, 
           Est..x, p.x, Est..y, p.y, Est., p) %>%
    mutate(across(contains("Est"), ~comma(.x, accuracy = 0.001)),
