---
title: "Post-COVID PC CRH Analysis: Effect of Removing PCP/AP FTE"
date: "`r Sys.Date()`"
...
* This is a sensitivity analysis examines the effect of removing __team PCP/APP FTE__ from the models
...
# An aside about collinearity  

```{r}
analytic_df2 %>%
  select(adi_natRnk_avg, avg_age_oct1_2022, avg_driveTime, nosos_risk_score, team_pcp_ap_fte_total,
         pct_male, pct_white, pct_rural) %>% 
...
wbm_fxn <- function(outcome){
  fmla_og <- as.formula(paste0(outcome, " ~ treated_month + time_t ", 
                               ...
                               " | (1|sta5a)"))
  fmla_no_fte <- as.formula(paste0(outcome, " ~ treated_month + time_t ", 
                                   "+ gap_metric_cat + adi_natRnk_avg + avg_age_oct1_2022 + avg_driveTime + nosos_risk_score",
                                   " + pct_male + pct_white + pct_rural + scrssn_count_cat", 
                                   "| treated_ever", 
                                   "+ gap_metric_cat + adi_natRnk_avg + avg_age_oct1_2022 + avg_driveTime + nosos_risk_score",
                                   " + pct_male + pct_white + pct_rural + s_abbr2 + scrssn_count_cat + parent_visn", 
                                   " | (1|sta5a)"))
  #
  mod_og <- 
  ...
  between_og_sum <- if({{outcome}} == "new_0_20_binary_success"){mod_og$between_table %>%
      ...
      rownames_to_column()}
  #
  within_no_fte_sum <- if({{outcome}} == "new_0_20_binary_success"){mod_no_fte$within_table %>%
      rownames_to_column() %>%
      mutate(`Est.` = exp(`Est.`))}
  else {mod_no_fte$within_table %>%
      rownames_to_column()}
  #
  between_no_fte_sum <- if({{outcome}} == "new_0_20_binary_success"){mod_no_fte$between_table %>%
      rownames_to_column() %>%
      mutate(`Est.` = exp(`Est.`))}
  else {mod_no_fte$between_table %>%
      rownames_to_column()}
  #----
  within_table <- within_og_sum %>%
    full_join(., within_no_fte_sum,
              by = "rowname") %>%
    select(rowname, 
           Est..x, p.x, Est..y, p.y) %>%
    ...
    add_header_above(c(" ", "Original Analysis" = 2, "No Team PCP/AP FTE" = 2),
                     bold = T,
                     font_size = 18)
  #=======
  between_table <-  between_og_sum %>%
    full_join(., between_no_fte_sum,
              by = "rowname") %>%
    select(rowname, 
           Est..x, p.x, Est..y, p.y) %>%
    mutate(across(contains("Est"), ~comma(.x, accuracy = 0.001)),
