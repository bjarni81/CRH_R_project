---
title: "Post-COVID PC CRH Analysis: Gap Metric X2"
date: "`r Sys.Date()`"
...
analytic_df2 <- vast %>%
  ...
  mutate(treated_ever = relevel(as.factor(treated_ever), ref = "0"),
         ...
         gap_metric2_cat = factor(case_when(gap_metric2 >= 1.2 ~ "> 1.2",
                                    gap_metric2 < 1 ~ "< 1.0",
                                    gap_metric2 < 1.2 & gap_metric2 >= 1.0 ~ "1.0-1.2")),
...
analytic_panel_noMatch$gap_metri2_cat <- relevel(analytic_panel_noMatch$gap_metric2_cat, ref = "< 1.0")
...
wbm_fxn <- function(outcome){
  fmla_og <- as.formula(paste0(outcome, " ~ treated_month + time_t ", 
                               ...
                               "+ crh_corr_fte + pct_male + pct_white + pct_rural + scrssn_count_cat", 
                               "| treated_ever", 
                               "+ gap_metric_cat + adi_natRnk_avg + avg_age_oct1_2022 + avg_driveTime + nosos_risk_score",
                               "+ crh_corr_fte + pct_male + pct_white + pct_rural + s_abbr2 + scrssn_count_cat + parent_visn", 
                               " | (1|sta5a)"))
  fmla_gap_metric2 <- as.formula(paste0(outcome, " ~ treated_month + time_t ", 
                               "+ gap_metric2_cat + adi_natRnk_avg + avg_age_oct1_2022 + avg_driveTime + nosos_risk_score",
                               "+ crh_corr_fte + pct_male + pct_white + pct_rural + scrssn_count_cat", 
                               "| treated_ever", 
                               "+ gap_metric2_cat + adi_natRnk_avg + avg_age_oct1_2022 + avg_driveTime + nosos_risk_score",
                               "+ crh_corr_fte + pct_male + pct_white + pct_rural + s_abbr2 + scrssn_count_cat + parent_visn", 
  ...
  between_og_sum <- if({{outcome}} == "new_0_20_binary_success"){mod_og %>% summary %$% between_table %>%
      ...
      rownames_to_column()}
  #
  within_gap_metric2_sum <- if({{outcome}} == "new_0_20_binary_success"){mod_gap_metric2 %>% summary %$% within_table %>%
      rownames_to_column() %>%
      mutate(`Est.` = exp(`Est.`))}
  else {mod_gap_metric2 %>% summary %$% within_table %>%
      rownames_to_column()}
  #
  between_gap_metric2_sum <- if({{outcome}} == "new_0_20_binary_success"){mod_gap_metric2 %>% summary %$% between_table %>%
      rownames_to_column() %>%
      mutate(`Est.` = exp(`Est.`))}
  else {mod_gap_metric2 %>% summary %$% between_table %>%
      rownames_to_column()}
  #----
  within_table <- within_og_sum %>%
    full_join(., within_gap_metric2_sum,
              by = "rowname") %>%
    select(rowname, 
           Est..x, p.x, Est..y, p.y) %>%
    ...
    add_header_above(c(" ", "Gap Metric 1" = 2, "Gap Metric 2" = 2),
                     bold = T,
                     font_size = 18)
  #=======
  between_table <-  between_og_sum %>%
    full_join(., between_gap_metric2_sum,
              by = "rowname") %>%
    select(rowname, 
           Est..x, p.x, Est..y, p.y) %>%
    mutate(across(contains("Est"), ~comma(.x, accuracy = 0.001)),
  ...
  #
  # aic_tab <- tibble(col_1 = c("Gap Metric 1", "Gap Metric 2"),
  #        col_2 = c(comma(AIC(mod_og), accuracy = 0.1), comma(AIC(mod_gap_metric2), accuracy = 0.1)),
  #        col_3 = c(comma(BIC(mod_og), accuracy = 0.1), comma(BIC(mod_gap_metric2), accuracy = 0.1))) %>%
  #   kbl(col.names = c("", "AIC", "BIC"),
  #       align = c("l", "r", "r")) %>%
  #   kable_classic("striped",
  #                 full_width = F) %>%
  #   row_spec(0, bold = TRUE)
