...
* We are using last observation carried forward for __age__ 
* We are using __CRH-corrected FTE__
* We are using __Gap Metric 2__
* We have dropped __drive time__
...
wbm_fxn <- function(outcome){
  wb_fmla <- as.formula(paste0(outcome, " ~ treated_month + time_t ", 
                               ...
                               " | (1|sta5a)"))
  #
  wb_mod <- if({{outcome}} == "new_0_20_binary_success"){panelr::wbm(wb_fmla,
                                                                     data = analytic_panel_noMatch,
                                                                     family = "binomial",
                                                                     pvals = T,
                                                                     pR2 = F,
                                                                     control = glmerControl(calc.derivs = FALSE))}
  else{panelr::wbm(wb_fmla, 
                   data = analytic_panel_noMatch)}
  #
  summary(wb_mod, digits = 3)
}
```
...
```{r}
wbm_fxn("est_pc_pt_wt_pid")
```

# New Patient Wait Time  
  
```{r}
wbm_fxn("new_pc_pt_wt_create")
```
...
wb_mod  %>% summary %$% 
  within_table %>%
  rownames_to_column() %>%
  mutate(oddsRatio = exp(`Est.`),
         lb_ci = exp(`Est.` - (1.96 * `S.E.`)),
         ub_ci = exp(`Est.` + (1.96 * `S.E.`)),
         across(where(is.numeric), ~ comma(.x, accuracy = 0.001)),
         conf_int = paste0(oddsRatio, " (", lb_ci, ", ", ub_ci, ")")) %>%
  select(rowname, conf_int)
#--
wb_mod  %>% summary %$% 
  between_table %>%
  rownames_to_column() %>%
