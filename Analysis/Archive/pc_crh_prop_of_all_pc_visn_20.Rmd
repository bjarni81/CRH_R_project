---
title: "PC CRH & Total PC Over Time - VISN 20"
date: "`r Sys.Date()`"
...
```{r}
visn_20_sta5as <- penRate %>%
  filter(parent_visn == "20" & crh_flag == 1) %>%
  select(sta5a) %>%
  distinct %>% pull
...
#======
penRate %>%
  filter(crh_flag == 1 & parent_visn == "20" & sta5a %in% visn_20_sta5as[1:8]) %>%
...
#======
penRate %>%
  filter(crh_flag == 1 & parent_visn == "20" & sta5a %in% visn_20_sta5as[9:15]) %>%
  group_by(fy_qtr, sta5a_lab) %>%
  summarise("PC CRH" = sum(crh_encounter_count, na.rm = T),
            "Total PC" = sum(pc_encounter_total, na.rm = T)) %>%
  pivot_longer(-c(fy_qtr, sta5a_lab)) %>%
...
# VISN 20 III  
* Note that Spokane went to Cerner starting Q2 of FY2021
...
#======
penRate %>%
  filter(crh_flag == 1 & parent_visn == "20" & sta5a %in% visn_20_sta5as[16:22]) %>%
  group_by(fy_qtr, sta5a_lab) %>%
  summarise("PC CRH" = sum(crh_encounter_count, na.rm = T),
            "Total PC" = sum(pc_encounter_total, na.rm = T)) %>%
  pivot_longer(-c(fy_qtr, sta5a_lab)) %>%
...
#======
penRate %>%
  filter(crh_flag == 1 & parent_visn == "20" & sta5a %in% visn_20_sta5as[23:30]) %>%
  group_by(fy_qtr, sta5a_lab) %>%
  summarise("PC CRH" = sum(crh_encounter_count, na.rm = T),
            "Total PC" = sum(pc_encounter_total, na.rm = T)) %>%
  pivot_longer(-c(fy_qtr, sta5a_lab)) %>%
