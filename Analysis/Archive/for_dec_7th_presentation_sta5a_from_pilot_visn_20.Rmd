---
title: "Primary Care CRH Encounters & Primary Care Access Metrics - VISN 20"
date: "`r Sys.Date()`"
...
```{r}
type_sta5a_df %>%
  filter(parent_visn == 20 & avg_crh_per_qtr > 100 & care_type == "Primary Care") %>% 
  select(-care_type) %>%
...
```{r}
total_sta5a_df %>%
  filter(parent_visn == 20 & avg_crh_per_qtr <= 100) %>% 
  mutate(sta5a_lab = fct_reorder2(factor(sta5a_lab),
                                         fyqtr, crh_per_1k_total)) %>%
  select(-c(sta5a, parent_visn, short_name)) %>%
  pivot_longer(-c(fyqtr, sta5a_lab)) %>%
...
### As a Table  
```{r}
total_sta5a_df %>% filter(parent_visn == 20) %>%
