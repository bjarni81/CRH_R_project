---
title: "Primary Care CRH Encounters & Primary Care Access Metrics - VISN 19"
date: "`r Sys.Date()`"
...
```{r}
type_sta5a_df %>%
  filter(parent_visn == 19 & avg_crh_per_qtr > 100 & care_type == "Primary Care") %>% 
  select(-care_type) %>%
...
```{r}
total_sta5a_df %>%
  filter(parent_visn == 19 & avg_crh_per_qtr <= 100) %>% 
  mutate(sta5a_lab = fct_reorder2(factor(sta5a_lab),
                                         fyqtr, crh_per_1k_total)) %>%
  select(-c(sta5a, parent_visn, short_name)) %>%
  pivot_longer(-c(fyqtr, sta5a_lab)) %>%
...
### As a Table  
```{r}
total_sta5a_df %>% filter(parent_visn == 19) %>%
...
access_metrics_sta5a %>%
  filter(parent_visn == 19 & care_type == "Primary Care") %>%
  select(fy_qtr, sta5a_lab, established_pt_waitTime, avg_crh_per_qtr) %>%
  mutate(fy_qtr = str_replace(fy_qtr, "_", "-"),
         sta5a_lab = fct_reorder2(factor(sta5a_lab),
                                         fy_qtr, established_pt_waitTime)) %>%
  filter(avg_crh_per_qtr > 100) %>%
  ggplot(data = .,
         aes(x = fy_qtr, y = established_pt_waitTime, color = sta5a_lab, group = sta5a_lab)) +
  geom_line(size = 1) +
  scale_y_continuous(limits = c(0, NA),
                     labels = scales::comma_format(accuracy = 1)) +
  theme(axis.text.x = element_text(angle = 270, vjust = 0.3)) +
...
### Established Patient Wait Time - As a Table  
```{r}
access_metrics_sta5a %>%
  ...
  mutate(established_pt_waitTime = format(established_pt_waitTime, 
                                          big.mark = ",", digits = 2),
         new_pt_waitTime = format(new_pt_waitTime, 
                                          big.mark = ",", digits = 2),
         obs_exp_panelSize_ratio = format(obs_exp_panelSize_ratio, 
                                          big.mark = ",", digits = 1),
         tc_pc_success_prop = paste0(round(tc_pc_success_prop * 100, 1), "%")) %>%
  pivot_longer(-c(fy_qtr, sta5a_lab)) %>%
  mutate(value = if_else(str_trim(value) == "NA", NA_character_, value)) %>%
...
access_metrics_sta5a %>%
  filter(parent_visn == 19 & care_type == "Primary Care") %>%
  select(fy_qtr, sta5a_lab, new_pt_waitTime, avg_crh_per_qtr) %>%
  mutate(fy_qtr = str_replace(fy_qtr, "_", "-"),
         sta5a_lab = fct_reorder2(factor(sta5a_lab),
                                         fy_qtr, new_pt_waitTime)) %>%
  filter(avg_crh_per_qtr > 100) %>%
  ggplot(data = .,
         aes(x = fy_qtr, y = new_pt_waitTime, color = sta5a_lab, group = sta5a_lab)) +
  geom_line(size = 1) +
  scale_y_continuous(limits = c(0, NA),
                     labels = scales::comma_format(accuracy = 1)) +
  theme(axis.text.x = element_text(angle = 270, vjust = 0.3)) +
...
### New Patient Wait Time - As a Table  
```{r}
access_metrics_sta5a %>%
  filter(parent_visn == 19) %>%
  select(fy_qtr, sta5a_lab, established_pt_waitTime, new_pt_waitTime,
...
access_metrics_sta5a %>%
  filter(parent_visn == 19 & care_type == "Primary Care") %>%
  select(fy_qtr, sta5a_lab, obs_exp_panelSize_ratio, avg_crh_per_qtr) %>%
  mutate(fy_qtr = str_replace(fy_qtr, "_", "-"),
         sta5a_lab = fct_reorder2(factor(sta5a_lab),
                                         fy_qtr, obs_exp_panelSize_ratio)) %>%
  filter(avg_crh_per_qtr > 100) %>%
  ggplot(data = .,
         aes(x = fy_qtr, y = obs_exp_panelSize_ratio, color = sta5a_lab, group = sta5a_lab)) +
  geom_line(size = 1) +
  scale_y_continuous(limits = c(0, NA),
                     labels = scales::comma_format(accuracy = 0.1)) +
  theme(axis.text.x = element_text(angle = 270, vjust = 0.3)) +
...
### Observed:Expected Panel Size Ratio - As a Table  
```{r}
access_metrics_sta5a %>%
  filter(parent_visn == 19) %>%
  select(fy_qtr, sta5a_lab, established_pt_waitTime, new_pt_waitTime,
  ...
  mutate(established_pt_waitTime = format(established_pt_waitTime, 
         ...
         tc_pc_success_prop = paste0(round(tc_pc_success_prop * 100, 1), "%")) %>%
  pivot_longer(-c(fy_qtr, sta5a_lab)) %>%
  filter(name == "obs_exp_panelSize_ratio") %>%
  pivot_wider(names_from = c(fy_qtr, name), values_from = value) %>%
  kbl(col.names = c("Sta5a", "2020-1", "2020-2", "2020-3", "2020-4",
                              "2021-1", "2021-2", "2021-3", "2021-4"),
      align = c("l", rep("r", 8))) %>%
...
access_metrics_sta5a %>%
  filter(parent_visn == 19 & care_type == "Primary Care") %>%
  select(fy_qtr, sta5a_lab, tc_pc_success_prop, avg_crh_per_qtr) %>%
  mutate(fy_qtr = str_replace(fy_qtr, "_", "-"),
         sta5a_lab = fct_reorder2(factor(sta5a_lab),
                                         fy_qtr, tc_pc_success_prop)) %>%
  filter(avg_crh_per_qtr > 100) %>%
  ggplot(data = .,
         aes(x = fy_qtr, y = tc_pc_success_prop, color = sta5a_lab, group = sta5a_lab)) +
  geom_line(size = 1) +
...
### Timely Care Success Rate - As a Table  
```{r}
access_metrics_sta5a %>%
  filter(parent_visn == 19) %>%
  select(fy_qtr, sta5a_lab, established_pt_waitTime, new_pt_waitTime,
  ...
  mutate(established_pt_waitTime = format(established_pt_waitTime, 
         ...
         obs_exp_panelSize_ratio = format(obs_exp_panelSize_ratio, 
                                          big.mark = ",", digits = 1),
         tc_pc_success_prop = paste0(format(round(tc_pc_success_prop * 100, 1), 
                                            nsmall = 1), "%")) %>%
  pivot_longer(-c(fy_qtr, sta5a_lab)) %>%
