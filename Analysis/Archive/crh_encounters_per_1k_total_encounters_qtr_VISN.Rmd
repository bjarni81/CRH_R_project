...
type_visn_df %>% filter(care_type == "Mental Health") %>%
  mutate(parent_visn_char = fct_reorder2(factor(str_pad(parent_visn, side = "left", width = 2, pad = 0)),
                                         fyqtr, crh_encounters)) %>%
  ggplot(data = .,
         aes(x = fyqtr, y = crh_encounters, group = parent_visn_char, color = parent_visn_char)) +
  geom_line(size = 1) +
  scale_y_continuous(limits = c(0, NA),
                     labels = scales::comma_format(accuracy = 1)) +
  theme(legend.title = element_blank(),
  ...
  guides(color = guide_legend(override.aes = list(size = 1.25))) +
  labs(x = "FY-Quarter",
       y = "CRH Mental Health Encounters")
```
...
type_visn_df %>% filter(care_type == "Mental Health") %>%
  mutate(parent_visn_char = fct_reorder2(factor(str_pad(parent_visn, side = "left", width = 2, pad = 0)),
                                         fyqtr, total)) %>%
  ggplot(data = .,
         aes(x = fyqtr, y = total, group = parent_visn_char, color = parent_visn_char)) +
  geom_line(size = 1) +
  scale_y_continuous(limits = c(0, NA),
                     labels = scales::comma_format(accuracy = 1)) +
  theme(legend.title = element_blank(),
