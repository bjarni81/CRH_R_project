---
title: "PC CRH & Total PC Over Time - VISN 06"
date: "`r Sys.Date()`"
...
```{r}
visn_06_sta5as <- penRate %>%
  filter(parent_visn == "06" & crh_flag == 1) %>%
  select(sta5a) %>%
  distinct %>% pull
...
#======
penRate %>%
  filter(crh_flag == 1 & parent_visn == "06" & sta5a %in% visn_06_sta5as[1:5]) %>%
...
#======
penRate %>%
  filter(crh_flag == 1 & parent_visn == "06" & sta5a %ni% visn_06_sta5as[1:5]) %>%
  group_by(fy_qtr, sta5a_lab) %>%
  summarise("PC CRH" = sum(crh_encounter_count, na.rm = T),
            "Total PC" = sum(pc_encounter_total, na.rm = T)) %>%
  pivot_longer(-c(fy_qtr, sta5a_lab)) %>%
