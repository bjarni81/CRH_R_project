...
total_df <- crh_nat_total %>%
  left_join(., all_nat_total, by = c("fyqtr" = "fyqtr")) %>%
  mutate(crh_per_1k_total = crh_encounters / total * 1000)
#
p1_tot <- ggplot(data = total_df,
         aes(x = fyqtr, y = crh_per_1k_total, color = "1", group = "1")) +
  geom_line(size = 1) +
...
p2_tot <- ggplot(data = total_df,
         aes(x = fyqtr, y = crh_encounters, color = "1", group = "1")) +
  geom_line(size = 1) +
  scale_y_continuous(limits = c(0, NA),
                     labels = scales::comma_format(accuracy = 1)) +
...
p3_tot <- ggplot(data = total_df,
         aes(x = fyqtr, y = total, color = "1", group = "1")) +
  geom_line(size = 1) +
...
type_nat_df <- crh_nat_type %>%
  left_join(., all_nat_type, by = c("fyqtr" = "fyqtr", "care_type" = "name")) %>%
  mutate(crh_per_1k_total = crh_encounters / total * 1000)
#
p1_mh <- ggplot(data = subset(type_nat_df, care_type == "Mental Health"),
         aes(x = fyqtr, y = crh_per_1k_total, group = "1", color = "1")) +
  geom_line(size = 1) +
...
p2_mh <- ggplot(data = subset(type_nat_df, care_type == "Mental Health"),
         aes(x = fyqtr, y = crh_encounters, group = "1", color = "1")) +
  geom_line(size = 1) +
  scale_y_continuous(limits = c(0, NA),
                     labels = scales::comma_format(accuracy = 1)) +
  scale_color_manual(values = aaas_pal5[2]) +
...
p3_mh <- ggplot(data = subset(type_nat_df, care_type == "Mental Health"),
         aes(x = fyqtr, y = total, group = "1", color = "1")) +
  geom_line(size = 1) +
  scale_y_continuous(limits = c(0, NA),
                     labels = scales::comma_format(accuracy = 1)) +
  scale_color_manual(values = aaas_pal5[3]) +
...
type_nat_df %>%
  mutate(crh_per_1k_total = round(crh_per_1k_total, 1)) %>%
  pivot_longer(-c(fyqtr, care_type)) %>%
  pivot_wider(names_from = c(care_type, name), values_from = value) %>%
  select(fyqtr, contains("Mental"), contains("Primary")) %>%
...
type_nat_df %>%
  ...
  pivot_longer(-c(fyqtr, care_type)) %>%
  pivot_wider(names_from = c(care_type, name), values_from = value) %>%
  select(fyqtr, contains("Pharm"), contains("PCMHI")) %>%
  kbl(col.names = c("Quarter",
                    rep(c("CRH", "Total", "CRH/1,000"), 2)),
      align = c("l", rep("r", 6)),
      format.args = list(big.mark = ",")) %>%
