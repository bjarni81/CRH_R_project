...
* Per our discussion on 6/5 I am running these analyses without the high leverage sta5a and it's match
...
* Description of analyses  
    ...
    + In addition to the various combinations of _treat_, _time_, and _post_, all models have the following covariates:
        ...
        + ADI
        + NOSOS
    + I have removed 436GH and it's match 554 from the Established and New Patient wait times analyses  
...
```{r}
bline_1_aug %>% filter(leverage.overall > 0.1) %>%
  left_join(., dates) %>%
  left_join(., vast) %>%
  mutate(sta5a_name = paste0("(", sta5a, ") ", short_name)) %>%
  select(sta5a_name, treat, vssc_month, est_pc_pt_wt) %>%
  kbl(col.names = c("Sta5a", "Treated/Control", "Observation Month", "Established Patient Wait Time")) %>%
...
```{r}
bline_2_aug %>% filter(leverage.overall > 0.1) %>%
  left_join(., dates) %>%
  left_join(., vast) %>%
  mutate(sta5a_name = paste0("(", sta5a, ") ", short_name)) %>%
  select(sta5a_name, treat, vssc_month, new_pc_pt_wt) %>%
  kbl(col.names = c("Sta5a", "Treated/Control", "Observation Month", "New Patient Wait Time")) %>%
  kable_classic("striped")
```
...
```{r}
bline_3_aug %>% filter(leverage.overall > 0.1) %>%
  left_join(., dates) %>%
  left_join(., vast) %>%
  mutate(sta5a_name = paste0("(", sta5a, ") ", short_name)) %>%
  select(sta5a_name, treat, vssc_month, third_next_avail) %>%
  kbl(col.names = c("Sta5a", "Treated/Control", "Observation Month", "Third Next Available")) %>%
  kable_classic("striped")
```
