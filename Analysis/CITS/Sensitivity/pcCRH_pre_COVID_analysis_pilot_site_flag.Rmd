...
analytic_df <- access_metrics %>%# start with access metrics
  ...
  mutate(treat = factor(case_when(
    ...
    pilot_site = factor(if_else(sta5a %in% pilot_sites, "Pilot Site", "Not a Pilot Site"))) %>%
  ungroup %>%
...
* This is a sensitivity analysis of the three pre-COVID analyses where I introduce a flag for whether the sta5a was a pilot site or not

## Decision Points  

* Per our discussion on 7/10:
    ...
    + This analysis includes a flag for pilot site
...
* This model includes _time2_ (i.e., months) as well as an interaction between _time2_ and _treat_, an interaction between _treat_ and _post_, as well as a 3-way interaction between _time2_, _treat_, and _post_
...
```{r}
bline_trend1 <- lmer(est_pc_pt_wt_pid ~ treat * post * time2  
                     + pilot_site
                     + obs_exp_panel_ratio
                     + pct_male
                     + pct_rural
