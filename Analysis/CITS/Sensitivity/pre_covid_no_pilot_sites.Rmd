---
title: "Sensitivity Analysis: Eliminating V-Impact sites pre-COVID"
date: "`r Sys.Date()`"
...
*  This is a sensitivity analysis of our pre-COVID time period where I've eliminated sites that were V-Impact pilot sites
...
plot_fxn <- function(outcome){
  ...
  p_w_pilots <- ggplot() +
    ...
    labs(title = "With Pilots")
  #
  p_wo_pilots <- ggplot() +
    geom_line(data = subset(p_df1, sta5a %ni% pilots_and_matches_to_drop),
              aes(x = vssc_month, y = {{outcome}}, group = sta5a),
              alpha = 0.3) +
    geom_smooth(data = subset(p_df1, sta5a %ni% pilots_and_matches_to_drop),
                aes(x = vssc_month, y = {{outcome}}, color = crh_flag),
                method = "lm",
                se = F) +
    geom_line(data = subset(p_df2, sta5a %ni% pilots_and_matches_to_drop),
              aes(x = vssc_month, y = {{outcome}}, group = sta5a),
              alpha = 0.3) +
    geom_smooth(data = subset(p_df2, sta5a %ni% pilots_and_matches_to_drop),
                aes(x = vssc_month, y = {{outcome}}, color = crh_flag),
                method = "lm",
                se = F) +
    theme(axis.title = element_blank()) +
    ggsci::scale_color_lancet() +
    labs(title = "Without Pilots")
  #--
  multiplot(p_w_pilots, p_wo_pilots, cols = 2)
}
```
...
```{r}
  w_pilots <- lmer(est_pc_pt_wt ~ treat * post    
                       + time * treat 
                       + obs_exp_panel_ratio
                       + pct_male
                       + pct_rural
         ...
         REML = TRUE)
  #
  wo_pilots <- lmer(est_pc_pt_wt ~ treat * post
                       + time * treat
                       + obs_exp_panel_ratio
                       + pct_male
                       + pct_rural
                       + (1|sta5a),
  data = subset(analytic_df,sta5a %ni% pilots_and_matches_to_drop),
  REML = TRUE)
  #--
  tab_model(w_pilots, wo_pilots,
          dv.labels = c("With Pilot Sites", "Without Pilot Sites"))
```
...
```{r}
new_w_pilots <- lmer(new_pc_pt_wt ~ treat * post    
                     + time * treat 
                     + obs_exp_panel_ratio
                     + pct_male
                     + pct_rural
                     ...
                     REML = TRUE)
#
new_wo_pilots <- lmer(new_pc_pt_wt ~ treat * post    
                     + time * treat 
                     + obs_exp_panel_ratio
                     + pct_male
                     + pct_rural
                     ...
                     REML = TRUE)
#--
tab_model(new_w_pilots, new_wo_pilots,
          dv.labels = c("With Pilot Sites", "Without Pilot Sites"))
```
...
```{r}
tna_w_pilots <- lmer(third_next_avail ~ treat * post    
                     + time * treat 
                     + obs_exp_panel_ratio
                     + pct_male
                     + pct_rural
                     ...
                     REML = TRUE)
#
tna_wo_pilots <- lmer(third_next_avail ~ treat * post    
                     + time * treat 
                     + obs_exp_panel_ratio
                     + pct_male
                     + pct_rural
                     ...
                     REML = TRUE)
#--
tab_model(tna_w_pilots, tna_wo_pilots,
          dv.labels = c("With Pilot Sites", "Without Pilot Sites"))
```