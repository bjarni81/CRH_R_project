---
title: "3-way Interaction - pre-COVID, with time and Matching, no Covariates"
date: "`r Sys.Date()`"
...
* After out meeting on 12/21/2022 we decided that one of our next analyses is 3-way interaction model, with interactions between __treat__, __post__, and __time__:  

$$y_{outcome}=\beta_{0} + \beta_{1}time_{t} +  \beta_{2}post_{t}+\beta_{3}treat_{j}+\beta_{4}time_{t}treat_{j}+\beta_{5}post_{t}treat_{j} + \beta_{6}time_{t}post_{t}treat_{j} +v_{j}+u_{jt}$$
...
* Description of analyses  
    + We will use a mixed-effects model with __REML__ 
    ...
    + On the RHS of the model we have:
        ...
        + Time (1-17; i.e., months)
        + An interaction between Treatment, Time, and Post
...
```{r}
bline_mean_1 <- lmer(new_pc_pt_wt ~ treat * post * time
                     + (1|sta5a),
                     data = analytic_df,
                     REML = TRUE)
```
...
```{r}
bline_mean_2 <- lmer(est_pc_pt_wt ~ treat * post * time
                     + (1|sta5a),
                     data = analytic_df,
                     REML = TRUE)
```
...
```{r}
bline_mean_3 <- lmer(third_next_avail ~ treat * post * time
                     + (1|sta5a),
                     data = analytic_df,
                     REML = TRUE)
```
