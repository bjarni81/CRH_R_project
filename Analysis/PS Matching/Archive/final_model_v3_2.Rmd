---
title: "PC CRH Analysis: Final PS Matching Model v3.2"
date: "`r Sys.Date()`"
...
source(here("Input", "Analytic df", "F2_propensity_score_matching","propensity_score_matching_v3_2_w_slopes.R"))
```
...
* From a starting set of `r sta5a_demog %>% nrow()` candidate sta5as, we drop  `r sta5a_demog %>% nrow() - sta5a_demog_no_missing %>% nrow()` sta5as that:
    ...
    + Have third Next available > 60 days (`r sta5a_demog %>% drop_na() %>% filter(third_next_avail_slope_fy19 > 60) %>% nrow()`)
    + Have Observed:Expected Panel Size > 5 (`r sta5a_demog %>% drop_na() %>% filter(obs_exp_panel_ratio_3mo_avg > 5) %>% nrow()`)
* This leaves `r nrow(sta5a_demog_no_missing)` sta5as, of which `r sum(sta5a_demog_no_missing$at_least_10_pc_crh_flag == 1)` met our inclusion criteria for PC CRH  
    + This seems reasonable, as the pre-drop proportion of all sta5as that met CRH criteria was `r (sum(sta5a_demog$at_least_10_pc_crh_flag == 1) / nrow(sta5a_demog)) %>% percent(accuracy = 0.1)` (`r sum(sta5a_demog$at_least_10_pc_crh_flag == 1)` out of `r nrow(sta5a_demog)`), and after it is `r (sum(sta5a_demog_no_missing$at_least_10_pc_crh_flag == 1) / nrow(sta5a_demog_no_missing)) %>% percent(accuracy = 0.1)` (`r sum(sta5a_demog_no_missing$at_least_10_pc_crh_flag == 1)` out of `r nrow(sta5a_demog_no_missing)`)
...
-   Model: Treatment (i.e., at least two consecutive months with \>9 PC CRH encounters) =
    ...
    -   Third Next Available PC Appointment (FY19 slope)
    -   Established Patient Waittime (FY19 slope)
    -   New Patient Waittime (FY19 slope)
