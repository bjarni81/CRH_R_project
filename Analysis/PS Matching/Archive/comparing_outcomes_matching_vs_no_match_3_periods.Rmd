...
ps_matched <- read_csv("H:/HSRD_General/Kaboli Access Team/CRH Evaluation/Bjarni/CRH_r_project/Input/Data/ps_matched_sta5as_v3.csv")
#pulling timely care
...
crh_sta5as <- crh_flag %>%
  ...
  pull
#
ps_matched_initiated_before_feb_20 <- crh_flag %>%
  filter(period_initiating_crh %in% 
           c("Initiated PC CRH Between 10/2019 and 2/2020", "Initiated PC CRH Before 9/2019")) %>%
  select(sta5a) %>%
...
analytic_df <- access_metrics %>%# start with access metrics
      ...
      filter(vssc_month < ymd("2020-03-01") & vssc_month > ymd("2018-06-01")) %>%#restrict to our timeframe
...
analytic_df2 <- access_metrics %>%# start with access metrics
      inner_join(., sta5as_to_include) %>%#restrict to our table from above
      filter(vssc_month < ymd("2020-03-01") & vssc_month > ymd("2018-06-01") & third_next_avail < 100) %>%#restrict to our timeframe
      left_join(., dates)
```
...
* Description of _Propensity Score_ matching
    + The current PS matching scheme is the naive one I proposed on November 1st
...
# New Patient Wait Time (ignoring values > 75)  
## Without Matching  
```{r}
ggplot(data = subset(analytic_df2, new_pc_pt_wt < 75),
  ...
  geom_smooth(size = 1.5, data = subset(analytic_df2, vssc_month < ymd("2018-10-01")),
...
## Without Matching (ignoring values < 60)    
```{r}
