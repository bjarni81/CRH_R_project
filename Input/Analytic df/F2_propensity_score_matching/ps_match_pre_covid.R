...
sta5a_demog <- dbGetQuery(oabi_con,
	...
	, b.pcmm_scrssn_count
	, c.est_pc_pt_wt_avg, c.new_pc_pt_wt_avg
	, c.est_pc_pt_wt_pid_avg, c.third_next_avail_avg, c.tc_success_prop_avg, c.tc_pc_success_prop_avg
	, d.adi_natRnk_avg
	, e.avg_non_va_cons, e.avg_tot_cons, e.cons_rate_avg, e.non_va_cons_rate_per_10k_pcmm
	, f.rural_prop
	, at_least_10_pc_crh_flag = case 
		when first_mo_w_mt9_pc_crh < cast('2020-03-01' as date) then 1
		when not_enough_crh_before_march_20 = 1 then NULL
		else 0 end
	, vast.parent_visn, vast.s_abbr, vast.station_name
	, crh_flag.not_enough_crh_before_march_20
...
left join [OABI_MyVAAccess].[crh_eval].C2_vast_from_a06 as vast
	on a.sta5a = vast.sta5a") %>%#mutate variables
  mutate(avg_tot_cons = replace_na(avg_tot_cons, 0),
         not_enough_crh_before_march_20 = replace_na(not_enough_crh_before_march_20, 0),
         non_va_cons_rate_per_10k_pcmm = replace_na(non_va_cons_rate_per_10k_pcmm, 0),
         at_least_10_pc_crh_flag = replace_na(at_least_10_pc_crh_flag, 0),
  ...
  mutate(pilot_visn_flag = if_else(parent_visn %in% c("06", "16", "19", "20"), "Pilot VISN", "Not a Pilot VISN"),
         mobile_flag = if_else(str_detect(station_name, "Mobile") == T
                               & s_abbr == "OOS", # catching Mobile, AL
                               1, 0)) %>%
  select(pcmm_scrssn_count
         ...
         , est_pc_pt_wt_pid_avg, new_pc_pt_wt_avg, third_next_avail_avg, s_abbr2,
         parent_visn, s_abbr, not_enough_crh_before_march_20, mobile_flag) %>%
  drop_na() # drop all rows with miissing observations
#Applying restrictions
sta5a_demog_no_missing <- sta5a_demog %>%
  filter(pcmm_scrssn_count > 450
         & not_enough_crh_before_march_20 == 0
         & mobile_flag == 0)
#--
write_csv(sta5a_demog_no_missing,
          here("Input", "Data", "sta5a_demog_no_missing.csv"))
#==================
...
#           here("Input", "Data", "ps_matched_sta5as_pre_covid.csv"))
