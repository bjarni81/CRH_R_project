...
#==================
opt_match_pcmm_noVISN <- matchit(at_least_10_pc_crh_flag ~  
                                   pilot_visn_flag
                                 + parent_visn
                                 + s_abbr2
                                 + pcmm_scrssn_count
                                 ...
                                 max.controls = 4,
                                 min.controls = 1,
                                 exact = ~pilot_visn_flag + s_abbr2)
#--
opt_matches_pcmm_noVISN <- match.data(opt_match_pcmm_noVISN) %>%
  select(sta5a, subclass, at_least_10_pc_crh_flag)
#--
opt_matches_pcmm1_noVISN <- opt_matches_pcmm_noVISN %>%
  select(sta5a, subclass, at_least_10_pc_crh_flag)
#======================
# commenting-out so that I can call base::source() without re-writing the .csv
# write_csv(opt_matches_pcmm1_noVISN,
#           here("Input", "Data", "ps_matched_sta5as_v4_wVISN_variable_ratio.csv"))
