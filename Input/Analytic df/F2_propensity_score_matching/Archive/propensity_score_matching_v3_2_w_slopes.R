...
sta5a_demog <- dbGetQuery(oabi_con,
	...
	, b.pcmm_scrssn_count
	, c.est_pc_pt_wt_slope_fy19, c.new_pc_pt_wt_slope_fy19, c.third_next_avail_slope_fy19--, c.tc_success_prop_avg, c.tc_pc_success_prop_avg
	, d.adi_natRnk_avg
...
left join [OABI_MyVAAccess].[crh_eval].F2_pcmm_scrssn_count as b
	on a.sta5a = b.sta5a
left join [OABI_MyVAAccess].[crh_eval].F3C_access_metrics_FY19_slope as c
	on a.sta5a = c.sta5a
...
left join [OABI_MyVAAccess].[crh_eval].C2_vast_from_a06 as vast
  ...
  select(pcmm_scrssn_count
         ...
         est_pc_pt_wt_slope_fy19, new_pc_pt_wt_slope_fy19, third_next_avail_slope_fy19, s_abbr2) %>%
  drop_na()
#
sta5a_demog_no_missing <- sta5a_demog %>%
  filter(pcmm_scrssn_count > 450 & nosos_3mo_avg < 2 & non_va_cons_rate_per_10k_pcmm < 50
         & third_next_avail_slope_fy19 < 60
         & s_abbr2 == "CBOC")
#==================
...
+ third_next_avail_slope_fy19
+ new_pc_pt_wt_slope_fy19
+ est_pc_pt_wt_slope_fy19,
...
#--
opt_matches_pcmm1 <- opt_matches_pcmm %>%
  select(sta5a, subclass, at_least_10_pc_crh_flag)
#======================
# commenting-out so that I can call base::source() without re-writing the .csv
# write_csv(opt_matches_pcmm1,
#           here("Input", "Data", "ps_matched_sta5as_v3_2.csv"))
