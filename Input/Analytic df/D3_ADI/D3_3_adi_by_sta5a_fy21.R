...
#patients in PCMM in fy21, Q1
patients_fy21_q1 <- dbGetQuery(oabi_connect,
                               "
    select *
    from [crh_eval].D3_TEMP_lat_lon_fy21
      where qtr = 1
        AND LAT IS NOT NULL
        AND LON IS NOT NULL
...
distinct_counties <- patients_fy21_q1 %>% distinct(STATEFP, COUNTYFP) %>% arrange(STATEFP, COUNTYFP)
################################################
...
patients_shp_fy21_q1 <- SpatialPointsDataFrame(coords = xy, 
                                               data = patients_fy21_q1,
...
#
# TEST: join_pssg_block(STATEFP_var = "36", COUNTYFP_var = "001", fy_var = 21, qtr_var = 1)
#--
...
geocoded_patients_fy21_q1 <- bind_rows(stack_blocks) %>% 
  filter(!is.na(fips)) %>%
  left_join(., patients_fy21_q1 %>% select(ScrSSN_num, Sta5a))
#--
fy_21_q1_time <- tictoc::toc()
print(paste0("fy21, Q1 took ", round((fy_21_q1_time$toc - fy_21_q1_time$tic) / 60, 1), " minutes"))
############################################
...
#---===---===---===
patients_adi_fy21_q1 <- geocoded_patients_fy21_q1 %>%
  left_join(., adi) %>%
...
#
sta5a_summary_fy21_q1 <- patients_adi_fy21_q1 %>%
...
############################################################################################################
rm(patients_fy21_q1)
rm(patients_shp_fy21_q1)
rm(patients_adi_fy21_q1)
rm(xy)
rm(stack_blocks)
rm(geocoded_patients_fy21_q1)
#
#patients in PCMM in fy21, Q2
tictoc::tic()
#
patients_fy21_q2 <- dbGetQuery(oabi_connect,
                               "
    select *
    from [crh_eval].D3_TEMP_lat_lon_fy21
      where qtr = 2
...
xy <- patients_fy21_q2 %>%
  select(LON, LAT)
# SpatialPointsDataFrame
patients_shp_fy21_q2 <- SpatialPointsDataFrame(coords = xy, 
                                               data = patients_fy21_q2,
                                               proj4string = CRS(raster::projection(block_shp_18)))
#---===---===---===---===---===---===---===---===---===---===
# loop over all counties - fy21, Q2
stack_blocks <- list() # To fill with geocoded data
COUNTYFP_old = "" # For printing
for(ii in seq_along(distinct_counties$STATEFP)){
  ...
  COUNTYFP_old = COUNTYFP_loop}
# Stack list of tract tibbles
geocoded_patients_fy21_q2 <- bind_rows(stack_blocks) %>% 
  filter(!is.na(fips)) %>%
  left_join(., patients_fy21_q2 %>% select(ScrSSN_num, Sta5a))
#--
fy_21_q2_time <- tictoc::toc()
print(paste0("fy21, Q2 took ", round((fy_21_q2_time$toc - fy_21_q2_time$tic) / 60, 1), " minutes"))
#---===---===---===---===---===---===---===---===---===---===---===---===---===---===---===---===---===---===
# ADI
patients_adi_fy21_q2 <- geocoded_patients_fy21_q2 %>%
  left_join(., adi) %>%
  mutate(in_1_25 = if_else(adi_natrank < 26, 1, 0),
         in_26_50 = if_else(adi_natrank > 25 & adi_natrank < 51, 1, 0),
         in_51_75 = if_else(adi_natrank > 50 & adi_natrank < 76, 1, 0),
         in_76_100 = if_else(adi_natrank > 75, 1, 0))
#
sta5a_summary_fy21_q2 <- patients_adi_fy21_q2 %>%
  group_by(Sta5a, fy, qtr) %>%
  summarise(adi_count = n(),
            adi_natRnk_avg = mean(adi_natrank, na.rm = T),
            adi_natRnk_sd = sd(adi_natrank, na.rm = T),
...
############################################################################################################
rm(patients_fy21_q2)
rm(patients_shp_fy21_q2)
rm(patients_adi_fy21_q2)
rm(xy)
rm(stack_blocks)
rm(geocoded_patients_fy21_q2)
#
#patients in PCMM in fy21, q3
tictoc::tic()
#
patients_fy21_q3 <- dbGetQuery(oabi_connect,
                               "
    select *
    from [crh_eval].D3_TEMP_lat_lon_fy21
      where qtr = 3
...
xy <- patients_fy21_q3 %>%
  select(LON, LAT)
# SpatialPointsDataFrame
patients_shp_fy21_q3 <- SpatialPointsDataFrame(coords = xy, 
                                               data = patients_fy21_q3,
                                               proj4string = CRS(raster::projection(block_shp_18)))
#---===---===---===---===---===---===---===---===---===---===
# fy21, q3
stack_blocks <- list() # To fill with geocoded data
...
for(ii in seq_along(distinct_counties$STATEFP)){
  ...
  COUNTYFP_old = COUNTYFP_loop}
# Stack list of tract tibbles
geocoded_patients_fy21_q3 <- bind_rows(stack_blocks) %>% 
  filter(!is.na(fips)) %>%
  left_join(., patients_fy21_q3 %>% select(ScrSSN_num, Sta5a))
#--
fy_21_q3_time <- tictoc::toc()
print(paste0("fy21, q3 took ", round((fy_21_q3_time$toc - fy_21_q3_time$tic) / 60, 1), " minutes"))
#---===---===---===---===---===---===---===---===---===---===---===---===---===---===---===---===---===---===
# ADI
patients_adi_fy21_q3 <- geocoded_patients_fy21_q3 %>%
  left_join(., adi) %>%
  mutate(in_1_25 = if_else(adi_natrank < 26, 1, 0),
         in_26_50 = if_else(adi_natrank > 25 & adi_natrank < 51, 1, 0),
         in_51_75 = if_else(adi_natrank > 50 & adi_natrank < 76, 1, 0),
         in_76_100 = if_else(adi_natrank > 75, 1, 0))
#
sta5a_summary_fy21_q3 <- patients_adi_fy21_q3 %>%
  group_by(Sta5a, fy, qtr) %>%
  summarise(adi_count = n(),
            adi_natRnk_avg = mean(adi_natrank, na.rm = T),
            adi_natRnk_sd = sd(adi_natrank, na.rm = T),
...
############################################################################################################
rm(patients_fy21_q3)
rm(patients_shp_fy21_q3)
rm(patients_adi_fy21_q3)
rm(xy)
rm(stack_blocks)
rm(geocoded_patients_fy21_q3)
#
#patients in PCMM in fy21, q4
tictoc::tic()
#
patients_fy21_q4 <- dbGetQuery(oabi_connect,
                               "
    select *
    from [crh_eval].D3_TEMP_lat_lon_fy21
      where qtr = 4
...
xy <- patients_fy21_q4 %>%
  select(LON, LAT)
# SpatialPointsDataFrame
patients_shp_fy21_q4 <- SpatialPointsDataFrame(coords = xy, 
                                               data = patients_fy21_q4,
                                               proj4string = CRS(raster::projection(block_shp_18)))
#---===---===---===---===---===---===---===---===---===---===
# fy21, q4
stack_blocks <- list() # To fill with geocoded data
...
for(ii in seq_along(distinct_counties$STATEFP)){
  ...
  COUNTYFP_old = COUNTYFP_loop}
# Stack list of tract tibbles
geocoded_patients_fy21_q4 <- bind_rows(stack_blocks) %>% 
  filter(!is.na(fips)) %>%
  left_join(., patients_fy21_q4 %>% select(ScrSSN_num, Sta5a))
#--
fy_21_q4_time <- tictoc::toc()
print(paste0("fy21, q4 took ", round((fy_21_q4_time$toc - fy_21_q4_time$tic) / 60, 1), " minutes"))
#---===---===---===---===---===---===---===---===---===---===---===---===---===---===---===---===---===---===
# ADI
patients_adi_fy21_q4 <- geocoded_patients_fy21_q4 %>%
  left_join(., adi) %>%
  mutate(in_1_25 = if_else(adi_natrank < 26, 1, 0),
         in_26_50 = if_else(adi_natrank > 25 & adi_natrank < 51, 1, 0),
         in_51_75 = if_else(adi_natrank > 50 & adi_natrank < 76, 1, 0),
         in_76_100 = if_else(adi_natrank > 75, 1, 0))
#
sta5a_summary_fy21_q4 <- patients_adi_fy21_q4 %>%
  group_by(Sta5a, fy, qtr) %>%
  summarise(adi_count = n(),
            adi_natRnk_avg = mean(adi_natrank, na.rm = T),
            adi_natRnk_sd = sd(adi_natrank, na.rm = T),
...
DBI::dbWriteTable(conn = oabi_connect,
                  name = table_id,
                  value = sta5a_summary_fy21_q1,
                  append = TRUE)
##
DBI::dbWriteTable(conn = oabi_connect,
                  name = table_id,
                  value = sta5a_summary_fy21_q2,
                  append = TRUE)
##
DBI::dbWriteTable(conn = oabi_connect,
                  name = table_id,
                  value = sta5a_summary_fy21_q3,
                  append = TRUE)
##
DBI::dbWriteTable(conn = oabi_connect,
                  name = table_id,
                  value = sta5a_summary_fy21_q4,
                  append = TRUE)
#=========
rm(patients_fy21_q4)
rm(patients_shp_fy21_q4)
rm(patients_adi_fy21_q4)
rm(xy)
rm(stack_blocks)
rm(geocoded_patients_fy21_q4)