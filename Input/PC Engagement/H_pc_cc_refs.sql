...
DECLARE @CompStartDate datetime2(0) = convert(datetime2(0),'20201001'); -- Start of study period
DECLARE @CompEndDate datetime2(0) = convert(datetime2(0),'20241001'); -- End of study period
...
left join Dim.StopCode b on a.PrimaryStopCodeSID = b.StopCodeSID 
	 and b.stopcode in (322, 323, 350) -- substitute with your stop codes of interest
...
select a.sta3n
	, a.consultsid
	, b.requestservicesid
	, req.ServiceName as torequestservicename
	, a.patientsid
	, a.requestdatetime
into #consults
from con.Consult a
...
on a.ToRequestServicesid = b.RequestServiceSID
		and a.sta3n = b.sta3n
left join [CDWWork].[Dim].RequestService as req
	on a.ToRequestServiceSID = req.RequestServiceSID
...
with c1 as (
	select b.PatientSID,
		...
		lower(req.servicename) ToRequestServiceName,
	...
	left join [CDWWork].[Dim].RequestService as req
		on c.ToRequestServiceSID = req.RequestServiceSID
)
...
select a.*,
	datediff(day,approveddatetime,AppointmentMadeDateTime) as dts,
...
drop table if exists #foo--[OABI_MyVAAccess].[crh_eval].G_pc_communityCare_referrals;
--
select *
into #foo--[OABI_MyVAAccess].[crh_eval].G_pc_communityCare_referrals
from #tmp_cons_dtc9
where sta6a <> '*Missing*' 
	AND dts >= 0 
	AND dtc >= 0
...
from #foo
---------
drop table if exists [OABI_MyVAAccess].[pccrh_eng].H_pc_cc_refs
--
with cte as(
	select *, DATEFROMPARTS(YEAR(ActivityDateTime), MONTH(ActivityDateTime), '01') as acty_month
	from #foo
)
select count(*) as pc_cc_referrals, a.sta6a, a.acty_month
into [OABI_MyVAAccess].[pccrh_eng].H_pc_cc_refs
from cte as a
group by sta6a, acty_month
--42,946
/***********------------------------------------------------------------------------------------------------------------------
VIEW RESULTS 
***********/
