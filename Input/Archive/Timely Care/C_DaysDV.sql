...
into #seen
from [OABI_MyVAAccess].[crh_eval].B_seen;
--15,575,682
--^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
--=================
/*Step 3c: Output permanent DaysDV table*/
--=================
drop table if exists [OABI_MyVAAccess].[crh_eval].C_daysDV;
--
select ScrSSN
	, desiredappointmentdatetime
	, req_sta5a
...
into [OABI_MyVAAccess].[crh_eval].C_daysDV
from #foo
group by ScrSSN, desiredappointmentdatetime, req_sta5a;
--12,216,734
--=================
-- DATA CHECKS
select top 100 * from [OABI_MyVAAccess].[crh_eval].C_daysDV
--
...
--=================
drop table if exists [OABI_MyVAAccess].[crh_eval].C1_daysDV_month_sta5a;
--
select req_sta5a
	, viz_month
	, viz_fy = case 
...
into [OABI_MyVAAccess].[crh_eval].C1_daysDV_month_sta5a
from #monthly_daysDV;
--44,800
select top 100 * from [OABI_MyVAAccess].[crh_eval].C1_daysDV_month_sta5a
--^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
/*Step 5a: Summarise the proportion of timelyCare requests that are fulfilled to the sta5a-month
		-- This is also where we catch those people who made requests on Fridays or Saturdays*/
--=================
drop table if exists #monthly_daysDV_parent;
--
select count(*) as tc_requests_sum
	, sum(timelyCare_success) as tc_success_sum
	, sum(timelyCare_PC_success) as tc_pc_success_sum
...
into #monthly_daysDV_parent
from [OABI_MyVAAccess].[crh_eval].C_daysDV as a
left join [OABI_MyVAAccess].[crh_eval].VAST_from_A06_11jan21 as b
	on a.req_sta5a = b.sta5a
where req_sta5a IS NOT NULL--requiring that req_sta5a not be missing; could do inner-join with VAST
group by parent_station_sta5a, DATEFROMPARTS(year(desiredAppointmentDateTime), month(desiredAppointmentDateTime), '01')
order by DATEFROMPARTS(year(desiredAppointmentDateTime), month(desiredAppointmentDateTime), '01'), parent_station_sta5a;
--5,900
select top 100 * from #monthly_daysDV_parent
--=================
/*Step 5b: Calculate rolling 6-month average and output permanent table*/
--=================
drop table if exists [OABI_MyVAAccess].[crh_eval].C2_daysDV_month_parent_sta5a;
--
select parent_station_sta5a
	...
	, tc_success_prop
	, tc_pc_success_prop
	, AVG(tc_success_prop) OVER(partition by parent_station_sta5a order by viz_month rows 6 preceding) as tc_success_6mo_avg
	, AVG(tc_pc_success_prop) OVER(partition by parent_station_sta5a order by viz_month rows 6 preceding) as tc_pc_success_6mo_avg
into [OABI_MyVAAccess].[crh_eval].C2_daysDV_month_parent_sta5a
from #monthly_daysDV_parent;
--5,900
select top 100 * from [OABI_MyVAAccess].[crh_eval].C2_daysDV_month_parent_sta5a
--^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
/*Step 6a: Summarise the proportion of timelyCare requests that are fulfilled to the sta5a-month
		-- This is also where we catch those people who made requests on Fridays or Saturdays*/
--=================
drop table if exists #monthly_daysDV_visn;
--
select count(*) as tc_requests_sum
	, sum(timelyCare_success) as tc_success_sum
	, sum(timelyCare_PC_success) as tc_pc_success_sum
	...
	, DATEFROMPARTS(year(desiredAppointmentDateTime), month(desiredAppointmentDateTime), '01') as viz_month
	, b.parent_visn
into #monthly_daysDV_visn
from [OABI_MyVAAccess].[crh_eval].C_daysDV as a
left join [OABI_MyVAAccess].[crh_eval].VAST_from_A06_11jan21 as b
	on a.req_sta5a = b.sta5a
where req_sta5a IS NOT NULL--requiring that req_sta5a not be missing; could do inner-join with VAST
group by parent_visn, DATEFROMPARTS(year(desiredAppointmentDateTime), month(desiredAppointmentDateTime), '01')
order by DATEFROMPARTS(year(desiredAppointmentDateTime), month(desiredAppointmentDateTime), '01'), parent_visn;
--774
select top 100 * from #monthly_daysDV_visn
--=================
/*Step 6b: Calculate rolling 6-month average and output permanent table*/
--=================
drop table if exists [OABI_MyVAAccess].[crh_eval].C3_daysDV_month_visn;
--
select parent_visn
	, viz_month
	, viz_fy = case 
	...
	, tc_success_prop
	, tc_pc_success_prop
	, AVG(tc_success_prop) OVER(partition by parent_visn order by viz_month rows 6 preceding) as tc_success_6mo_avg
	, AVG(tc_pc_success_prop) OVER(partition by parent_visn order by viz_month rows 6 preceding) as tc_pc_success_6mo_avg
into [OABI_MyVAAccess].[crh_eval].C3_daysDV_month_visn
from #monthly_daysDV_visn;
--774
select top 100 * from [OABI_MyVAAccess].[crh_eval].C3_daysDV_month_visn
--^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
/*Step 7a: Summarise the proportion of timelyCare requests that are fulfilled to the sta5a-month
		-- This is also where we catch those people who made requests on Fridays or Saturdays*/
--=================
drop table if exists #monthly_daysDV_nat;
--
select count(*) as tc_requests_sum
	, sum(timelyCare_success) as tc_success_sum
	, sum(timelyCare_PC_success) as tc_pc_success_sum
	...
	, sum(cast(timelyCare_PC_success as float)) / cast(count(*) as float) as tc_pc_success_prop
	, DATEFROMPARTS(year(desiredAppointmentDateTime), month(desiredAppointmentDateTime), '01') as viz_month
into #monthly_daysDV_nat
from [OABI_MyVAAccess].[crh_eval].C_daysDV as a
left join [OABI_MyVAAccess].[crh_eval].VAST_from_A06_11jan21 as b
	on a.req_sta5a = b.sta5a
where req_sta5a IS NOT NULL--requiring that req_sta5a not be missing; could do inner-join with VAST
group by DATEFROMPARTS(year(desiredAppointmentDateTime), month(desiredAppointmentDateTime), '01')
order by DATEFROMPARTS(year(desiredAppointmentDateTime), month(desiredAppointmentDateTime), '01');
--43
select top 100 * from #monthly_daysDV_nat
--=================
/*Step 7b: Calculate rolling 6-month average and output permanent table*/
--=================
drop table if exists [OABI_MyVAAccess].[crh_eval].C4_daysDV_month_nat;
--
select
	viz_month
	, viz_fy = case 
	...
	, tc_success_prop
	, tc_pc_success_prop
	, AVG(tc_success_prop) OVER(order by viz_month rows 6 preceding) as tc_success_6mo_avg
	, AVG(tc_pc_success_prop) OVER(order by viz_month rows 6 preceding) as tc_pc_success_6mo_avg
into [OABI_MyVAAccess].[crh_eval].C4_daysDV_month_nat
from #monthly_daysDV_nat;
--43
select top 100 * from [OABI_MyVAAccess].[crh_eval].C4_daysDV_month_nat
--
select min(viz_month) from [OABI_MyVAAccess].[crh_eval].C4_daysDV_month_nat
--
select max(viz_month) from [OABI_MyVAAccess].[crh_eval].C4_daysDV_month_nat