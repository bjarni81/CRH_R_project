...
select distinct
	...
	, careType = case 
			when (a.PrimaryStopCode in (156, 176, 177, 178, 301, 322, 323, 338, 348) 
					AND (a.SecondaryStopCode not in(160, 534, 539) OR a.SecondaryStopCode IS NULL)) 
				OR (a.PrimaryStopCode not in(160, 534, 539) 
...
set @EndDate = CAST('2022-01-01' as datetime2);/*FY21*/
--+++++
...
select op.PatientSID, op.VisitDateTime
	...
	, b.careType
	, b.covid_vax_flag
	, b.pStopCode_, b.pStopCodeName_
	, b.sStopCode_, b.sStopCodeName_
	, fy = case when month(op.visitDateTime) > 9 then year(op.visitDateTime) + 1 else year(op.visitDateTime) end
	, qtr = case when month(op.visitDateTime) in(10, 11, 12) then 1
				when month(op.visitDateTime) in(1, 2, 3) then 2
				when month(op.visitDateTime) in(4, 5, 6) then 3
				when month(op.visitDateTime) in(7, 8, 9) then 4 end
...
order by vizMonth
--============================================================
-- Stop code combinations
drop table if exists [OABI_MyVAAccess].[crh_eval].stopCode_combos;
--
select count(*) as sc_count
	, careType
	, pStopCode_, pStopCodeName_
	, sStopCode_, sStopCodeName_
into [OABI_MyVAAccess].[crh_eval].stopCode_combos
from #outpat_encounters
where covid_vax_flag = 0
	and fy in(2020, 2021)
group by careType
	, pStopCode_, pStopCodeName_
	, sStopCode_, sStopCodeName_
...
--
select top 100 *
from #stopCode_combos
--============================================================
-- Stop code combinations by sta5a
drop table if exists [OABI_MyVAAccess].[crh_eval].stopCode_combos_sta5a;
--
select count(*) as sc_count
	, careType
	, pStopCode_, pStopCodeName_
	, sStopCode_, sStopCodeName_
	, Sta6a
into [OABI_MyVAAccess].[crh_eval].stopCode_combos_sta5a
from #outpat_encounters
where covid_vax_flag = 0
group by careType, Sta6a
	, pStopCode_, pStopCodeName_
	, sStopCode_, sStopCodeName_
;
--
select top 100 *
from [OABI_MyVAAccess].[crh_eval].stopCode_combos_sta5a
--============================================================
--ssc = 117 by sta5a
drop table if exists [OABI_MyVAAccess].[crh_eval].ssc_117_sta5a;
--
select count(*) as ssc_117_count
	, sta6a
into [OABI_MyVAAccess].[crh_eval].ssc_117_sta5a
from #outpat_encounters
where sStopCode_ = 117
	and careType = 'PC'
group by sta6a;
--
select top 100 *
