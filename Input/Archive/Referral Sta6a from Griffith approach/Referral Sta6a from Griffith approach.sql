...
DECLARE @CompStartDate datetime2(0) = convert(datetime2(0),'20181001'); -- Start of study period
DECLARE @CompEndDate datetime2(0) = convert(datetime2(0),'20220331'); -- End of study period
...
with c1 as (
	select b.PatientSID,
		...
		lower(c.torequestservicename) ToRequestServiceName,
...
select a.*,
	...
	left join [CDWWork].[Dim].Division as c
	on b.DivisionSID = c.DivisionSID

-- Cleanup temp tables as we go along
--drop table if exists #tmp_cons_dtc6, #tmp_cons_dtc7, #tmp_cons_dtc8;
--
select top 100 * from #tmp_cons_dtc9;
select count(distinct sta6a) from #tmp_cons_dtc9;
--703
/***********
VIEW RESULTS 
***********/
