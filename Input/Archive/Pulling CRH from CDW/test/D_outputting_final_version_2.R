...
crh_encounters_1 <- dbGetQuery(oabi_con,
                               "select *
                               from [OABI_MyVAAccess].[crh_eval].encounters_C_unioned_v2") %>%
  rename_all(tolower) %>%
  janitor::clean_names()
...
table_id <- DBI::Id(schema = "crh_eval", table = "encounters_D_deDup_v2")
#
dbSendQuery(oabi_con, "drop table if exists [OABI_MyVAAccess].[crh_eval].encounters_D_deDup_v2")
#foo
