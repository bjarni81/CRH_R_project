...
timely_care_daysDV <- sqlQuery(channel, 
                               "SELECT * FROM [crh_eval].C2_daysDV_month_parent_sta5a", 
                               stringsAsFactors=FALSE) %>%
  mutate(viz_month = ymd(viz_month)) %>%
  rename(req_sta5a = parent_station_sta5a)
#***&&&***&&&***&&&***
access_path <- list.files(path = here("Input", "Data", "VSSC"),
                          full.names = T, pattern = "parent")
#
avg_3rd_next_available <- read_xlsx(path = access_path[1],
  ...
  mutate(month.n = match(str_to_title(str_sub(name, end = 3)), month.abb),
         ...
         sta5a = if_else(str_sub(full_name, start = 12, end = 12) == ")",
                         str_sub(full_name, start = 9, end = 11),
                         str_sub(full_name, start = 9, end = 13))) %>%
...
write_csv(all_pc_access_metrics,
          here("Input", "Data", "pc_access_metrics_parent.csv"))
